...
GitHub default branch is the measured `laguna` branch, not the upstream mirror.
See [`BRANCHES.md`](BRANCHES.md) for every maintained and experimental Apathy
Atlas branch.
...
| `ds4-flash` | DeepSeek-V4-Flash/REAP | `bench/deepseek-v4/` | Single-GB10 model bring-up, prefill, decode, MTP and quality evidence. |
| `feat/tui-benchmarks` | Atlas infrastructure | plugin and TUI | Experimental benchmark UI and run-scoped state work. |
| `feat/qwen38-repro-public` | Qwen3.8-27B | `repro/qwen38-coding/` | Sanitized deterministic coding evidence; release inputs are not all public yet. |

Other remote branches are not implied Apathy Atlas release channels.

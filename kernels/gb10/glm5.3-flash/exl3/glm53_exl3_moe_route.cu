...
// Direct-combine pack additionally records the inverse stable permutation in
// already-owned route-private scratch. source_to_sorted[source] identifies the
// expert-major row while preserving source slot order for the final reduction.
extern "C" __global__ void atlas_glm53_exl3_pack_routes_private_inverse(
    const uint32_t* __restrict__ route_ids,
    const float* __restrict__ route_weights,
    int64_t* __restrict__ expert_count,
    int64_t* __restrict__ token_sorted,
    ...
    uint32_t experts,
    uint32_t top_k,
    uint32_t* __restrict__ source_to_sorted)
{
    if (blockIdx.x != 0) return;
    ...
    for (uint32_t tile = 0; tile < pairs; tile += blockDim.x)
    {
        ...
        if (source < pairs && selected < experts)
        {
            ...
            token_sorted[destination] = static_cast<int64_t>(source);
            weight_sorted[destination] = __float2half_rn(route_weights[source]);
            source_to_sorted[source] = destination;
        }
        __syncthreads();

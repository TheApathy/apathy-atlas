...
// The only arithmetic change is route-private final storage; all projection,
// Hadamard, activation, scale, scheduler, and weight traversal code is pinned.
...
extern "C" __global__
__launch_bounds__(EXL3_GEMM_BASE_THREADS * MOE_TILESIZE_K / 16)
void atlas_glm53_exl3_moe_private_k2_n256_cb2(EXL3_MOE_KERNEL_ARGS)
{
    constexpr int t_bits = 2;
    constexpr int MOE_TILESIZE_N = 256;
    constexpr int cb = 2;
    ...
    for (; expert_idx < num_experts; ++expert_idx)
    {
        ...
        auto had_gather_gu_in = [&]()
        {
            ...
            {
                int64_t source_pair = top_x[warp_idx / warps_per_token];
                int token_idx = source_pair / num_experts_per_tok;
        ...
        auto had_d_out = [&]()
        {
            ...
            for (int warp_idx = warp_idx0; warp_idx < total_warps; warp_idx += warps_per_group)
            {
                ...
                int token_off = warp_idx % warps_per_token;
                float* out_ptr = output_state + source_pair * hidden_dim + token_off * 128;

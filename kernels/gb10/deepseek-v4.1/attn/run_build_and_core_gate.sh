...
echo "$(date -u +%FT%TZ) dsv41-attention build (under lock, no GPU) + core gates runF/runG incl. decode (~3 min) QUEUED pid=$$" >> $Q
...
cargo build --release -j 16 -p spark-model --example dsv41_attn_seam --example dsv41_attn_replay_gate --features cuda,gpu-examples > $W/kernels/gb10/deepseek-v4.1/attn/build.log 2>&1; rb=$?
r1=-1; r2=-1; r3=-1
if [ $rb -eq 0 ]; then
...
echo "$(date -u +%FT%TZ) dsv41-attention build+core-gate END rc=build:$rb,runF:$r1,runG:$r2,runI:$r3 pid=$$" >> $Q
flock -u 9
echo "DONE build=$rb runF=$r1 runG=$r2 runI=$r3"

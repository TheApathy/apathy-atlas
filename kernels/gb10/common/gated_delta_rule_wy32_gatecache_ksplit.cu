...
//
// GDN K-split (R3): 512 threads, register-resident state.
//
// PORTED VERBATIM from bench/gdn/gdn_cand3.cu. The only edits are the entry-point
// rename and the cuda_bf16.h include the bench harness supplied externally; the
// arithmetic is untouched, so the harness receipts in bench/gdn/gdn_r3.txt and
// gdn_r36.txt describe THIS kernel. Its baseline there, bench/gdn/gdn_ref.cu, is
// byte-identical to gated_delta_rule_wy32_gatecache.cu, so the measured 1.23x and
// the 3.2e-05 output relative L2 are both against the kernel this one replaces.
//
// NOT BIT-EXACT: gated behind ATLAS_GDN_PREFILL_GATECACHE_KSPLIT=1. See below.
//

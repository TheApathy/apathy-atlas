...
// ============================================================================
// batchN variants (forward_k16 / wide DFlash verify). Byte-identical per-block
// math to batch2_t above (one block = one (token, expert_slot) GEMV) — the ONLY
// change is that the number of tokens is a runtime `num_tokens` param instead of
// the hardcoded 2, so all num_tokens*(top_k+1) blocks launch in ONE grid and
// fill the GPU (vs 16 serial per-token launches @ ~8% SM util). FAITHFUL: the
// GEMV/dequant is unchanged, so the target argmax matches the per-token path →
// speculative acceptance preserved. Grid: (ceil(N/BLOCK), num_tokens*(top_k+1), z).
// blockIdx.y: 0..num_tokens*top_k routed; num_tokens*top_k.. shared.
// ============================================================================
extern "C" __global__ void moe_expert_gate_up_shared_batchN_t(
...
extern "C" __global__ void moe_expert_silu_down_shared_batchN_t(

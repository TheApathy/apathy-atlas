...
// V4: byte-for-byte the V3 arithmetic and loop order. The ONLY change is that the
...
void qwen4_moe_compact_t_gemm_k32_v4(
    const __nv_bfloat16* __restrict__ A,
    const unsigned long long* __restrict__ packed_ptrs,
    const unsigned long long* __restrict__ scale_ptrs,
    const float* __restrict__ scale2_vals,
...
// (12 warps) to 4 (16 warps). This is the controlled occupancy arm -- nothing but
// CTAs/SM differs from V2. Bit-exact with V2.
extern "C" __global__ __launch_bounds__(128, 4)
void qwen4_moe_compact_t_gemm_k32_v5(
    const __nv_bfloat16* __restrict__ A,
    const unsigned long long* __restrict__ packed_ptrs,
    const unsigned long long* __restrict__ scale_ptrs,
    const float* __restrict__ scale2_vals,
    ...
    __shared__ __align__(16) unsigned char sB[STAGES][16][B_STRIDE];
    __shared__ __align__(16) unsigned char sS[STAGES][2][S_STRIDE];
    __shared__ __align__(16) __nv_bfloat16 sBf[oi640::STEP_K][BF_STRIDE];
    __shared__ float s_lut[16];

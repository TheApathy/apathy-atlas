...
//
// Compile-only SM121a EXL3 W2A8 grouped-prefill component.
//
...
// into registers. This file lives outside kernels/gb10/common on purpose;
// DeepSeek-only wrappers expose it solely through the explicit default-off
// serving experiment. GPU numeric and timing gates still block promotion.
...
static_assert(W2A8_FIXED_N % W2A8_N_TILE == 0, "N must be N64 aligned");
...
extern "C" __global__ __launch_bounds__(128) void W2A8_KERNEL_NAME(
    ...
    if (blockDim.x != 128 || blockDim.y != 1 || blockDim.z != 1) return;
    ...
    __shared__ __align__(16) uint4 smem_T[4][16];
...
#pragma unroll 1
            for (unsigned int k_stage = 0; k_stage < W2A8_SCALE_GROUP;
                ...
                for (unsigned int load = threadIdx.x; load < 64;
                     load += blockDim.x) {
                    const unsigned int k_tile = load >> 4;
                    const unsigned int word = load & 15;

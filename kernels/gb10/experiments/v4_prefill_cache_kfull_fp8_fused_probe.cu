...
// Standalone GPU promotion probe for the isolated strided-K-full cache fusion.
// This file is intentionally outside the Atlas registry and serving path.
...
#ifndef V4_CACHE_KFULL_PROBE_BUILD_ID
#error "V4_CACHE_KFULL_PROBE_BUILD_ID must bind this probe to a build receipt"
#endif

// BEGIN K-full cache probe kernel contract
...
constexpr unsigned kKHeadDim = 512;
constexpr unsigned kKNope = 448;
...
constexpr unsigned kExtractCtas = 603;
...
static_assert((kTokens * kRope + kThreads - 1) / kThreads == kExtractCtas,
              "K-tail extraction CTA count changed");
...
static void launch_baseline(
    ...
    mla_q_rope_extract_batched<<<dim3(kExtractCtas, 1, 1), dim3(kThreads, 1, 1)>>>(
        k_full, rope_scratch, kTokens, 1, kKHeadDim, kKNope, kRope, kKHeadDim);
    mla_cache_assemble_batched<<<dim3(kTokens, 1, 1), dim3(kCacheDim, 1, 1)>>>(
        latent, rope_scratch, k_scratch, v_scratch, kKvLora, kRope, kCacheDim);
...
static void launch_candidate(
    ...
    v4_prefill_cache_kfull_fp8_fused<<<dim3(kTokens, 1, 1), dim3(kThreads, 1, 1)>>>(
        latent, k_full, k_cache, v_cache, slots, kTokens, kNumBlocks,
        kBlockSize, k_scale, v_scale, kCacheStride);
}
// Baseline K-tail extract: incremental_bytes=616960 incremental_ctas=603.
// END K-full cache probe kernel contract
...
static uint64_t fnv(uint64_t hash, const void* data, size_t bytes) {
    ...
    for (size_t i = 0; i < bytes; ++i) hash = (hash ^ ptr[i]) * 1099511628211ull;
    return hash;
}
...
struct Guarded {
    ...
    void fill(unsigned char poison) { check(cudaMemset(data, poison, bytes()), "poison fill"); }
    ...
    bool guards_clean() const {
        ...
        for (unsigned char byte : prefix) if (byte != 0xa7) return false;
        for (unsigned char byte : suffix) if (byte != 0x7a) return false;
        return true;
    }
};
...
static uint16_t bf16_bits(float value) {
    ...
    if ((bits & 0x7f800000u) == 0x7f800000u) return static_cast<uint16_t>(bits >> 16);
...
struct ProbeState {
    ...
    Guarded<__nv_bfloat16> k_full{static_cast<size_t>(kTokens) * kKHeadDim};
    Guarded<long long> slots{kTokens};
    Guarded<__nv_bfloat16> rope_scratch{static_cast<size_t>(kTokens) * kRope};
...
// BEGIN K-full cache deterministic parity
// Contract: kParityCases = 3, kPoisonCases = 2, unique nonmonotonic slots.
...
static void make_case(const CaseSpec& spec,
                      ...
                      std::vector<__nv_bfloat16>& k_full,
                      std::vector<long long>& slots) {
    auto* latent_raw = reinterpret_cast<uint16_t*>(latent.data());
    auto* k_raw = reinterpret_cast<uint16_t*>(k_full.data());
    ...
    for (size_t i = 0; i < k_full.size(); ++i) {
        const int value = static_cast<int>((i * 193u + spec.k_salt * 23u) % 509u) - 254;
        k_raw[i] = bf16_bits(static_cast<float>(value) * (1.0f / 32.0f));
    }
...
static std::vector<unsigned char> validate_parity_slots(
    ...
    for (long long slot : slots) {
        ...
        if (seen[static_cast<size_t>(slot)] != 0) die("parity slot collision");
...
struct ParityResult {
    ...
    bool rope_scratch_guards_clean = true;
    bool assembly_scratch_guards_clean = true;
...
static ParityResult run_parity(ProbeState& state,
                               std::vector<__nv_bfloat16>& host_latent,
                               std::vector<__nv_bfloat16>& host_k_full,
                               std::vector<long long>& host_slots) {
    ParityResult result;
    for (const CaseSpec& spec : kCases) {
        make_case(spec, host_latent, host_k_full, host_slots);
        ...
        state.latent.upload(host_latent); state.k_full.upload(host_k_full); state.slots.upload(host_slots);
        result.input_hash = fnv(result.input_hash, host_latent.data(), host_latent.size() * sizeof(__nv_bfloat16));
        result.input_hash = fnv(result.input_hash, host_k_full.data(), host_k_full.size() * sizeof(__nv_bfloat16));
        result.slot_hash = fnv(result.slot_hash, host_slots.data(), host_slots.size() * sizeof(long long));
        ...
        for (int poison_pass = 0; poison_pass < kPoisonCases; ++poison_pass) {
            ...
            state.rope_scratch.fill(baseline_poison);
            ...
            launch_baseline(state.latent.data, state.k_full.data, state.rope_scratch.data,
                            state.k_scratch.data, state.v_scratch.data,
                            state.baseline.k.data, state.baseline.v.data,
                            state.slots.data, spec.kScale, spec.vScale);
            launch_candidate(state.latent.data, state.k_full.data, state.candidate.k.data,
                             state.candidate.v.data, state.slots.data, spec.kScale, spec.vScale);
            check(cudaGetLastError(), "parity launch"); check(cudaDeviceSynchronize(), "parity sync");
            ...
            result.rope_scratch_guards_clean &= state.rope_scratch.guards_clean();
            result.assembly_scratch_guards_clean &=
                state.k_scratch.guards_clean() && state.v_scratch.guards_clean();
            result.cache_guards_clean &= state.baseline.guards_clean() && state.candidate.guards_clean();
        }
    }
    if (result.k_mismatches || result.v_mismatches || !result.rope_scratch_guards_clean ||
        !result.assembly_scratch_guards_clean || !result.cache_guards_clean ||
        !state.latent.guards_clean() || !state.k_full.guards_clean() || !state.slots.guards_clean())
        die("K-full parity or guard failure");
    return result;
}
// END K-full cache deterministic parity

// BEGIN K-full cache malformed ABI
// Contract: kMalformedCases = 25 full-cache no-write checks.
...
static void expect_unchanged(CachePair& output, const CandidateArgs& args, const char* label) {
    output.fill(0x3c);
    const auto before_k = output.k.download(), before_v = output.v.download();
    v4_prefill_cache_kfull_fp8_fused<<<args.grid, args.block>>>(
        args.latent, args.k_full, args.k_cache, args.v_cache, args.slots,
        args.num_tokens, args.num_blocks, args.block_size, args.k_scale,
        args.v_scale, args.cache_stride);
    check(cudaGetLastError(), label); check(cudaDeviceSynchronize(), label);
...
static void run_malformed(ProbeState& state, std::vector<long long>& host_slots) {
    CandidateArgs base{state.latent.data, state.k_full.data, state.candidate.k.data,
                       state.candidate.v.data, state.slots.data};
    auto run = [&](CandidateArgs args, const char* label) { expect_unchanged(state.candidate, args, label); };
    { auto a=base; a.latent=nullptr; run(a,"null-latent"); }
    { auto a=base; a.k_full=nullptr; run(a,"null-k-full"); }
    ...
    { auto a=base; a.k_full=reinterpret_cast<const __nv_bfloat16*>(reinterpret_cast<const unsigned char*>(a.k_full)+2); run(a,"unaligned-k-full"); }
    ...
    state.slots.upload(host_slots); run(base,"out-of-pool-slot");
}
// END K-full cache malformed ABI

static double parse_speedup_threshold(int argc, char** argv) {
    if (argc != 2) { std::fprintf(stderr,"usage: %s <min_speedup>\n",argv[0]); std::exit(2); }
    const char* input=argv[1]; bool digit=false,dot=false,exponent=false,exponent_digit=false;
    for(size_t i=0;input[i]!='\0';++i){const unsigned char ch=static_cast<unsigned char>(input[i]);
        ...
        else if((ch=='+'||ch=='-')&&exponent&&!exponent_digit&&i>0&&(input[i-1]=='e'||input[i-1]=='E'))continue;
        else{std::fprintf(stderr,"invalid explicit numeric threshold\n");std::exit(2);}}
    if(!digit||(exponent&&!exponent_digit)){std::fprintf(stderr,"invalid explicit numeric threshold\n");std::exit(2);}
    errno=0;char* end=nullptr;const double min_speedup=std::strtod(input,&end);
    if(errno!=0||end==input||*end!='\0'||!std::isfinite(min_speedup)||min_speedup <= 1.0||min_speedup > 100.0){
        std::fprintf(stderr,"invalid explicit numeric threshold\n");std::exit(2);}return min_speedup;
}

// BEGIN K-full cache ABBA timing
// Contract: kWarmupRounds = 2, kAbbaRounds = 6; baseline_ms, candidate_ms,
// speedup use baseline, candidate, candidate, baseline event order.
struct TimingContext {
    ProbeState& state;
    const std::vector<__nv_bfloat16>& host_latent;
    const std::vector<__nv_bfloat16>& host_k_full;
    const std::vector<long long>& host_slots;
    const CaseSpec& spec;
    void reset_inputs(){state.latent.upload(host_latent);state.k_full.upload(host_k_full);state.slots.upload(host_slots);}
    void reset_outputs(bool baseline){if(baseline){state.rope_scratch.fill(kPoisonA);state.k_scratch.fill(kPoisonA);state.v_scratch.fill(kPoisonA);state.baseline.fill(kPoisonA);}else state.candidate.fill(kPoisonA);}
    float timed(bool baseline){cudaEvent_t start,stop;check(cudaEventCreate(&start),"event create");check(cudaEventCreate(&stop),"event create");
        auto reset=[&](){reset_inputs();reset_outputs(baseline);};reset();check(cudaEventRecord(start),"cudaEventRecord");
        if(baseline)launch_baseline(state.latent.data,state.k_full.data,state.rope_scratch.data,state.k_scratch.data,state.v_scratch.data,state.baseline.k.data,state.baseline.v.data,state.slots.data,spec.kScale,spec.vScale);
        else launch_candidate(state.latent.data,state.k_full.data,state.candidate.k.data,state.candidate.v.data,state.slots.data,spec.kScale,spec.vScale);
        check(cudaEventRecord(stop),"cudaEventRecord");check(cudaEventSynchronize(stop),"event sync");float ms=0;check(cudaEventElapsedTime(&ms,start,stop),"cudaEventElapsedTime");check(cudaEventDestroy(start),"destroy");check(cudaEventDestroy(stop),"destroy");return ms;}
};
static void time_abba(TimingContext& context,double& baseline_ms,double& candidate_ms){auto timed=[&](bool baseline){return context.timed(baseline);};
    for(int round=0;round<kWarmupRounds;++round){timed(true);timed(false);timed(false);timed(true);}baseline_ms=0;candidate_ms=0;
    for(int round=0;round<kAbbaRounds;++round){const float baseline_first=timed(true);const float candidate_first=timed(false);const float candidate_second=timed(false);const float baseline_second=timed(true);baseline_ms+=baseline_first+baseline_second;candidate_ms+=candidate_first+candidate_second;}
    baseline_ms/=2*kAbbaRounds;candidate_ms/=2*kAbbaRounds;}
// END K-full cache ABBA timing

static int run(int argc,char** argv){const double min_speedup=parse_speedup_threshold(argc,argv);
    int driver=0,runtime=0,device=0;cudaDeviceProp properties{};check(cudaDriverGetVersion(&driver),"driver");check(cudaRuntimeGetVersion(&runtime),"runtime");check(cudaGetDevice(&device),"device");check(cudaGetDeviceProperties(&properties,device),"properties");
    ProbeState state;std::vector<__nv_bfloat16> host_latent(static_cast<size_t>(kTokens)*kKvLora),host_k_full(static_cast<size_t>(kTokens)*kKHeadDim);std::vector<long long> host_slots(kTokens);
    const ParityResult parity=run_parity(state,host_latent,host_k_full,host_slots);run_malformed(state,host_slots);make_case(kCases[0],host_latent,host_k_full,host_slots);validate_parity_slots(host_slots);
    TimingContext timing{state,host_latent,host_k_full,host_slots,kCases[0]};double baseline_ms=0,candidate_ms=0;time_abba(timing,baseline_ms,candidate_ms);const double speedup=baseline_ms/candidate_ms;if(!std::isfinite(speedup)||speedup<min_speedup)die("pre-registered speedup threshold not met");
    char uuid[33];for(int i=0;i<16;++i)std::sprintf(uuid+2*i,"%02x",static_cast<unsigned char>(properties.uuid.bytes[i]));uuid[32]='\0';
    std::printf("build_id=%s\n",V4_CACHE_KFULL_PROBE_BUILD_ID);
    std::printf("device_uuid=%s driver=%d runtime=%d\n",uuid,driver,runtime);
    std::printf("input_hash=%016llx slot_hash=%016llx scale_hash=%016llx\n",static_cast<unsigned long long>(parity.input_hash),static_cast<unsigned long long>(parity.slot_hash),static_cast<unsigned long long>(parity.scale_hash));
    std::printf("parity cases=3 poison_cases=2 k_mismatches=0 v_mismatches=0\n");
    std::printf("guards rope_scratch=clean assembly_scratch=clean cache=clean malformed_cases=25 unchanged=25 incremental_bytes=616960 incremental_ctas=603\n");
    std::printf("timing baseline_ms=%.6f candidate_ms=%.6f speedup=%.9f abba_rounds=6\n",baseline_ms,candidate_ms,speedup);
    std::printf("threshold min_speedup=%.17g\n",min_speedup);std::printf("result=PASS\n");return 0;}
}  // namespace probe

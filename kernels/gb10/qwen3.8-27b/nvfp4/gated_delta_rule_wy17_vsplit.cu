...
//
// gated_delta_rule_wy17_lazy_vsplit combines this occupancy split with the
// LAZY Hi-write optimization from gated_delta_rule_wy17_lazy: intermediate
// state slots Hi_t are only written to DRAM for checkpoint positions (t==0,
// t==K-2, or (t+1)%lazy_j==0). Non-checkpoint slots are skipped; the commit
// path reconstructs them via the wy17_replay kernel. Saves 86% of PASS-2
// intermediate write traffic (with lazy_j=16: 2 slots instead of 16) while
// doubling CTA occupancy. Checkpoint logic is identical to wy17_lazy so the
// same replay kernel is compatible.
...
// Checkpoint predicate for LAZY Hi-writes (mirrors gated_delta_rule_wy17.cu).
// A slot s is a checkpoint iff: s==0 || s==K-2 || ((s+1)%lazy_j==0).
// lazy_j==1 → always a checkpoint (write all, bit-identical to non-lazy path).
__device__ __forceinline__ bool wy17vs_is_checkpoint(int s, unsigned int lazy_j) {
...
// Combined lazy Hi-writes + V-DIM SPLIT kernel.
// Identical to gated_delta_rule_wy17_vsplit but adds a lazy_j parameter that
// gates intermediate Hi_t writes to checkpoint slots only.  The output values
// and final h_state are bit-identical for any lazy_j.  Non-checkpoint slots
// are reconstructed by the wy17_replay kernel on partial accepts.
extern "C" __global__ void gated_delta_rule_wy17_lazy_vsplit(
    float* __restrict__ h_state,
    const __nv_bfloat16* __restrict__ query,
    const __nv_bfloat16* __restrict__ key,
    const __nv_bfloat16* __restrict__ value,
    ...
    if (active) {
        ...
        for (unsigned int j = 0; j < k_dim; j += 4) {
            ...
            for (int t = 0; t < K_TOKENS; t++) {
                ...
                h3 = sg[t] * h3 + sk[t][j + 3] * vn[t];
                if (t < K_TOKENS - 1) {
                    if (wy17vs_is_checkpoint(t, lazy_j)) {
                        float* Hi_t = Hi_base + t * inter_stride_floats;
                        Hi_t[(j + 0) * v_dim + vcol] = h0;
                        Hi_t[(j + 1) * v_dim + vcol] = h1;
                        Hi_t[(j + 2) * v_dim + vcol] = h2;

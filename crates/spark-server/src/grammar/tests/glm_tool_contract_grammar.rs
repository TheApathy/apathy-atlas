...
fn native_parser() -> Box<dyn crate::tool_parser::ToolCallParser> {
    ...
    let parser = format.into_parser();
    assert_eq!(parser.param_value_close_delim(), Some("</arg_value>"));
    parser
}
...
fn glm_tool_contract_grammar_rollback_restores_name_choice_masks_and_stop_legality() {
    let (compiled, size) = compile(&tools(), false);
    let mut state = GrammarState::new(&compiled, size)
        .unwrap()
    ...
    assert_eq!(
        state.num_history_steps(),
        accepted,
        "exempt EOS is not a rollback step"
    );

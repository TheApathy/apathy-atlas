...
fn images_require_supported_ownership_while_text_is_unchanged() {
    ...
    for (count, vision, concurrency, yarn) in [
        (1, false, 1, false),
        (1, true, 2, false),
        (1, true, 0, false),
        (1, true, 1, true),
...
fn request_guard_precedes_actual_prompt_preparation() {
    ...
    assert!(
        source.find("image_admission::validate(").unwrap()
            < source.find("prepare_chat_prompt(").unwrap()
    );
    let token = include_str!("../misc_handlers.rs");
    assert!(
        token.find("unsupported_multimodal_token_count").unwrap()
...
fn image_reset_preserves_codispatch_prepass_and_stream_fence() {
    ...
    let encode = chunk
        .find("model.prepare_vision_embed(&image_pixels)?;")
        .unwrap();
    let fence = chunk[encode..]
        .find("model.record_event(prefill_event, model.default_stream())?")
        .unwrap();
    assert!(guard < encode);
    assert!(fence > 0);

...
fn a_setting_the_recipe_does_not_list_can_be_added() {
    ...
    let args = r.serve_args(&overrides).expect("validates");
    // Both are plain values on this engine, not `Option`s resolved later:
    // `kv_cache_dtype` is a `String` (clap default "fp8") and
    // `fp8_kv_calibration_tokens` a `usize`. The override still has to land,
    // which is what this test is for — only the shape of the assert changes.
...
// REMOVED: `every_recipe_can_be_served_hermetically`.
//
// This engine has no `--hermetic` flag and no `cli::hermetic` module, so the
// test had nothing to exercise. It is deleted rather than ignored because an
// #[ignore]d test for an absent feature reads as a known failure someone
// should fix, when the truth is that the feature was never ported.
//
// Its guarantee is worth restoring WITH the flag: every vendored recipe must

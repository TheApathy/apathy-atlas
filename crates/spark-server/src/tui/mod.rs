...
pub struct RunHandles {
    ...
    // RULING B: this engine publishes the scheduler snapshot through a
    // process-global `scheduler::snapshot::{publish, read}` rather than
    // upstream's per-instance `SnapshotCell`. The DATA is identical
    // (`SchedulerSnapshot`, field for field); only the container differs, so
    // the dashboard reads the global directly and this handle is not carried.
    // Do not add `SnapshotCell` to the engine to satisfy a display.
}

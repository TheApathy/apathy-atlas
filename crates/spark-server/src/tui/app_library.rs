...
impl App {
    ...
    /// `recipes_unavailable`. `AtlasHome::resolve()` reads `ATLAS_HOME`/`HOME` and
    ...
    pub(super) fn attach_recipes(&mut self) {
        match crate::atlas_home::AtlasHome::resolve() {
            Ok(home) => {
                self.lib.attach(home.root().to_path_buf(), &self.library);
                self.lib.refresh();
            }
            Err(e) => {

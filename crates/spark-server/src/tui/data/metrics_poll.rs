...
impl StatsModel {
    ...
    pub fn sample(&mut self, _run: Option<&crate::tui::RunHandles>) {
        ...
        // The engine already records these at the two counted radix-tree
        // entrypoints; only the reader was missing. PROCESS-CUMULATIVE, not
        // per-run: upstream scopes them to the current model so a swap does
        // not carry the previous one's cache activity forward. With no swap
        // path here there is one model per process, so the two agree — when a
        // swap lands, this needs a reset at the same point.
        ...
        // `None` until something has been looked up: a 0.0 rate before the
        // first request reads as "missing every lookup" rather than "no data".

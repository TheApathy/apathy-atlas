        ...
        if !header_done {
            if let Some(pos) = find_subslice(&buf, b"\r\n\r\n") {
                ...
                if !head.starts_with("HTTP/1.1 200") && !head.starts_with("HTTP/1.0 200") {
                    ...
                    // The WHOLE response, headers included — see
                    // `error_message_from_body` for why it does not de-chunk.
                    let msg = error_message_from_body(&buf)
...
/// The human-readable message from a non-200 chat response body.
///
/// PORTED WITHOUT ITS DE-CHUNKER, DELIBERATELY. Upstream reads this through
/// `avarok_plugin::http`'s incremental reader and warns — correctly — that "a
/// second de-chunker written by hand is exactly how one caller ends up
/// understanding `14A\r\n{...}` and the other not". Rather than write that
/// second decoder, this does not decode framing at all: it takes the span from
/// the first `{` to the last `}` and parses THAT.
///
/// Why that is sound here: `transfer-encoding: chunked` puts its size lines
/// BETWEEN chunks, outside the JSON object, so for an error body that fits in
/// one chunk — which every OpenAI-shaped error does, and the read above caps at
/// `MAX_ERROR_BODY` anyway — the span is exactly the JSON.
///
/// And when it is NOT sound it FAILS VISIBLY rather than wrongly: a body split

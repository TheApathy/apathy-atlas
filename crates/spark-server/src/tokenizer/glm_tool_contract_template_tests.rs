...
//! JSON-only GLM template tests against the repo's GLM override template.
//! all-models port: the GLM branch's IR -> MsgEntry path tests need IR types
//! this tree does not have; their parallel-result ordering cases are kept
//! below as template-level cases over the JSON the message builder emits.

use super::{jinja_helpers, normalize_tool_call_arguments};
...
fn render(messages: &[Value], thinking: bool, tools: Option<&[Value]>) -> String {
...
fn parallel_history(with_ids: bool) -> Vec<Value> {
    let call = |id: &str, literal: &str| {
        json!({"id":id, "type":"function", "function":{"name":"inspect",
    ...
    vec![
        ...
        json!({"role":"assistant", "content":"",
            "tool_calls":[call("a", "first"), call("b", "second")]}),
        result("b", "RESULT_B"),
        result("a", "RESULT_A"),
    ]
}
...
#[test]
fn glm_tool_contract_parallel_results_render_in_call_order_by_id() {
    let rendered = render(&parallel_history(true), false, None);
    let first = rendered
...
fn glm_tool_contract_unassociated_results_keep_input_order_without_guessing() {
    let rendered = render(&parallel_history(false), false, None);

...
pub(super) fn policy_device_argmax_enabled_from(value: Option<&str>) -> Result<bool> {
    match value {
        None | Some("0") => Ok(false),
        Some("1") => Ok(true),
        Some(value) => anyhow::bail!(
            "ATLAS_GLM53_POLICY_DEVICE_ARGMAX must be absent or exactly 0 or 1; got {value:?}"
        ),
    }
}

fn policy_device_argmax_enabled() -> Result<bool> {
    match std::env::var("ATLAS_GLM53_POLICY_DEVICE_ARGMAX") {
        Ok(value) => policy_device_argmax_enabled_from(Some(&value)),
        Err(std::env::VarError::NotPresent) => policy_device_argmax_enabled_from(None),
        Err(error @ std::env::VarError::NotUnicode(_)) => {
            anyhow::bail!("ATLAS_GLM53_POLICY_DEVICE_ARGMAX must be valid UTF-8: {error}")
        }
    }
}
...
pub(in crate::scheduler) struct OrdinaryVerifyPolicy<'a> {
    ...
    compact_exclusions: Option<[u32; 2]>,
    closed: bool,
    failed: bool,
}
...
impl<'a> OrdinaryVerifyPolicy<'a> {
    pub(in crate::scheduler) fn new(
        ...
        let compact_exclusions = policy_device_argmax_enabled()?
            .then(|| {
                super::super::ordinary_greedy::policy_raw_argmax_exclusions(
                    seq,
                    context,
    ...
    pub(in crate::scheduler) fn compact_argmax_exclusions(&self) -> Option<[u32; 2]> {
        self.compact_exclusions
    }

    pub(in crate::scheduler) fn pick_argmax(&mut self, index: usize, token: u32) -> Result<u32> {
        ensure!(
            self.compact_exclusions.is_some()
                && self.saved.is_some()
                && !self.closed
                && !self.failed
                && !self.seq.finished,
            "GLM compact policy pick outside admitted checkpoint"
        );
        ensure!(
            ...
            "GLM compact policy token or row is invalid"
        );
        self.pending = Some((token, None));
        Ok(token)
    }

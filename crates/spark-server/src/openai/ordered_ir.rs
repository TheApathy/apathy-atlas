...
impl ChatCompletionRequest {
    ...
    pub(crate) fn validate_content_order(&self) -> Result<(), String> {
        for message in &self.messages {
            message.content.validate_order()?;
        }
        Ok(())
    }
}
...
impl ParsedContent {
    pub(crate) fn ordered_ir_parts(&self) -> Result<Vec<ContentPart>, String> {
        ...
        for (&end, image) in self.image_text_offsets.iter().zip(&self.images) {
            if end > start {
                parts.push(ContentPart::Text(self.text[start..end].to_string()));
            }
            parts.push(ContentPart::Image(ImageSource {
                data: ImageData::from_uri(image.clone()),
            }));
            start = end;
        }
        if start < self.text.len() {
            parts.push(ContentPart::Text(self.text[start..].to_string()));
        }
        Ok(parts)
    }
}
...
mod tests {
        ...
        assert_eq!(
            ...
            vec![
                ...
                ContentPart::Image(ImageSource {
                    data: ImageData::from_uri("red".into())
                }),
                ContentPart::Image(ImageSource {
                    data: ImageData::from_uri("blue".into())
                }),
    ...
    fn invalid_programmatic_content_is_rejected_before_conversion() {
        let mut content = ParsedContent {
            text: "🟥".into(),
            images: vec!["red".into()],
            image_text_offsets: vec![1],
        };
        assert!(content.ordered_ir_parts().is_err());
        content.image_text_offsets = vec![4];
        assert!(content.ordered_ir_parts().is_ok());
        let mut req: ChatCompletionRequest = serde_json::from_value(
            serde_json::json!({"model":"test","messages":[{"role":"user","content":"ok"}]}),
        )
        .unwrap();
    ...
    fn prepend_inserts_before_leading_image_and_leaves_later_text_in_place() {
        for tail in ["", "later"] {
            ...
            if !tail.is_empty() {
                assert_eq!(ir.content[2], ContentPart::Text(tail.into()));
            }

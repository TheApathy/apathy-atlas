...
impl ParsedContent {
    ...
    pub(super) fn parse(value: Value, responses: bool) -> Result<Self, String> {
        ...
        for part in parts {
            ...
            let kind = obj
                ...
                .ok_or("content part requires a type")?;
            match kind {
                "text" | "input_text" | "output_text" if kind == "text" || responses => {
                    let text = obj
                        .get("text")
                        .and_then(Value::as_str)
                        .ok_or("text part requires string text")?;
                    out.text.push_str(text);
                }
                "image_url" | "input_image" if kind == "image_url" || responses => {
                    let image = obj
                        .get("image_url")
                        .ok_or("image part requires image_url")?;
                    let url = if kind == "input_image" {
                        image.as_str()
                    } else {
                        image.get("url").and_then(Value::as_str)
                    }
                    .filter(|s| !s.is_empty())
                    .ok_or("image part requires a nonempty image URL")?;
    ...
    fn validate_offsets(text: &str, count: usize, offsets: &[usize]) -> Result<(), String> {
        ...
        for &offset in offsets {
            if offset < prior || offset > text.len() || !text.is_char_boundary(offset) {
                return Err(
                    "image offsets must be ordered UTF-8 byte boundaries within text".into(),
                );
            }
            prior = offset;
        }
    ...
    pub fn chat_json(&self) -> Result<Value, String> {
        ...
        Ok(Value::Array(Self::parts(
            &self.text,
            &self.image_text_offsets,
            "text",
            |i| json!({"type":"image_url", "image_url":{"url":self.images[i]}}),
        )))
    }
    ...
    pub fn responses_json(&self) -> Result<Value, String> {
        ...
        Ok(Value::Array(Self::parts(
            ...
            &self.image_text_offsets,
            "input_text",
            |i| json!({"type":"input_image", "image_url":self.images[i]}),
        )))
    }
}

...
fn markers_preserve_adjacent_unicode_order_without_payloads() {
    let c = ordered();
    let markers =
        ParsedContent::marker_json(&c.text, c.images.len(), &c.image_text_offsets).unwrap();
    assert_eq!(
        markers,
        json!([
...
fn chat_and_responses_roundtrip_all_positions() {
    ...
    let chat: IncomingMessage =
        serde_json::from_value(json!({"role":"tool","content":c.chat_json().unwrap()})).unwrap();
    let response = IncomingMessage::try_from_responses_input_item(
        &json!({"role":"tool","content":c.responses_json().unwrap()}),
    )
    .unwrap()
    .unwrap();
...
fn checked_responses_reject_malformed_instead_of_dropping() {
    for item in [
        ...
        assert!(
            IncomingMessage::try_from_responses_input_item(&item).is_err(),
            "{item}"
        );
    }
    assert!(
        IncomingMessage::try_from_responses_input_item(&json!({"type":"reasoning"}))
            .unwrap()
            .is_none()
    );
}
...
fn image_only_and_text_only_serialization_are_lossless() {
    ...
    assert_eq!(
        ParsedContent::marker_json("", 1, &[0]).unwrap(),
        json!([{"type":"image"}])
    );
    assert_eq!(ParsedContent::default().chat_json().unwrap(), json!(""));
    assert_eq!(
        ParsedContent::default().responses_json().unwrap(),

...
fn interleaved_images_keep_unicode_text_offsets() {
    ...
    assert_eq!(m.content.image_text_offsets, ["First 🟥:".len(), "First 🟥:Second:".len()]);
...
fn malformed_image_or_text_parts_are_rejected() {
    for part in [json!({"type":"image_url"}), json!({"type":"text"}), json!({"type":"input_audio","input_audio":{}})] {
        assert!(serde_json::from_value::<IncomingMessage>(json!({"role":"user","content":[part]})).is_err());
    }
}
...
fn plain_and_null_content_preserve_existing_semantics() {
    for (content, expected) in [(json!("Hello"), "Hello"), (json!(null), "")] {
        let m: IncomingMessage = serde_json::from_value(json!({"role":"assistant","content":content})).unwrap();

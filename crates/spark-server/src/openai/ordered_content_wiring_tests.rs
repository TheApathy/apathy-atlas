...
fn checked_order_precedes_each_wire_to_ir_edge() {
    for source in [
        include_str!("../api/chat/mod.rs"),
        include_str!("../api/responses.rs"),
        include_str!("../api/responses_stream.rs"),
    ] {
        let check = source.find(".validate_content_order()").unwrap();
        let conversion = source
            .find("req.into()")
            .or_else(|| source.find("chat_req.into()"))
            .unwrap();
...
fn image_ownership_is_not_compacted_or_deduplicated() {
    let template: String = include_str!("../api/chat/template.rs")
        .split_whitespace()
        .collect();
    ...
    let entries: String = include_str!("../api/chat/msg_entry.rs")
        .split_whitespace()
        .collect();
...
fn replay_and_persistence_preserve_order() {
    ...
    for source in [
        include_str!("../api/stored.rs"),
        include_str!("../api/responses_stream.rs"),
        include_str!("../api/responses_translate.rs"),
    ] {

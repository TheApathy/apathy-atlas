...
pub fn dsml_safe(name: &str) -> bool {
    !name.is_empty() && !name.chars().any(|c| c == '"' || c == '｜' || (c as u32) < 0x20 || c as u32 == 0x7f)
}
...
fn string_value(schema: &Value) -> String {
    if let Some(Value::Array(e)) = schema.get("enum")
        && !e.is_empty()
        && e.iter().all(|x| x.as_str().is_some_and(|s| !s.contains('｜')))
    {
...
pub fn build_tool_grammar(tools: &[Value], max_calls: usize) -> Option<String> {
    ...
    for (i, f) in functions(tools).into_iter().enumerate() {
        ...
        if let Some(Value::Object(props)) = props {
            for (j, (pname, pschema)) in props.iter().enumerate() {
                if !dsml_safe(pname) {
                    tracing::warn!("parameter {pname:?} of tool {name:?} cannot be expressed in DSML; left out");
                    continue;
                }
                ...
                let body = if let Some(jrule) = jrule {
                    ...
                    need_vany = true;
                    format!("{} (\"true\" | \"false\") {} vany", lit(&format!("{head}{q}")), lit(&format!("{q}>")))
                };
        ...
        let body = if seq.is_empty() { String::new() } else { format!(" {}", seq.join(" ")) };
        rules.push(format!("{rid} ::= {open_tag}{body} {}", lit(&invoke_close())));
        calls.push(rid);
    }
    ...
    let mut parts = vec![
        format!("# DSML tool calls for {} tool(s); generated by server/tool_grammar.py", calls.len()),
        format!("root ::= {} call{{1,{}}} {}", lit(&block_open()), max_calls.max(1), lit(&block_close())),

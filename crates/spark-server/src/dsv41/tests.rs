...
fn render_fixtures_match_python_byte_and_token_exact() {
    ...
    for c in cases(&fx) {
        ...
        if c.get("error").is_some() {
            assert!(got.is_err(), "{name}: Python refused ({}) but Rust rendered", c["error"]["message"]);
            errs += 1;
            continue;
        }
        let got = got.unwrap_or_else(|e| panic!("{name}: Rust refused: {e}"));
        assert_eq!(got.thinking, c["thinking"].as_bool().unwrap(), "{name}: thinking");
        assert_eq!(got.effort as u64, c["effort"].as_u64().unwrap(), "{name}: effort");
        let want = c["prompt"].as_str().unwrap();
        assert_eq!(got.prompt, want, "{name}: prompt differs");
        let want_ids: Vec<u32> =
            c["ids"].as_array().unwrap().iter().map(|x| x.as_u64().unwrap() as u32).collect();
        assert_eq!(encode(&tok, &got.prompt), want_ids, "{name}: token ids differ");
        assert_eq!(got.images.len() as u64, c["n_images"].as_u64().unwrap(), "{name}: images");
        let placeholders = want_ids.iter().filter(|&&t| t == 129264).count();
        assert_eq!(placeholders, got.images.len(), "{name}: placeholder tokens vs images");
        let gt = got.grammar_tools.clone().map(Value::Array).unwrap_or(Value::Null);
        assert_eq!(gt, c["grammar_tools"], "{name}: grammar tools");
        ok += 1;
    }
    assert!(ok >= 30 && errs >= 8, "fixture coverage shrank: {ok} rendered, {errs} errors");
}
...
fn render_comparison_detects_plausible_port_mistakes() {
    ...
    let rendered: Vec<(String, String)> = cases(&fx)
        ...
        .map(|c| (c["name"].as_str().unwrap().to_string(), c["prompt"].as_str().unwrap().to_string()))
        .collect();
    let mistakes: [(&str, fn(&str) -> String); 5] = [
        ("V4 DSML spelling (no leading space)", |p| p.replace("｜DSML｜ ", "｜DSML｜")),
        ...
        ("tool results unsorted (a<->c)", |p| {
            p.replace("<tool_result>18C</tool_result>", "@@")
                .replace("<tool_result>headline 1\nheadline 2</tool_result>", "<tool_result>18C</tool_result>")
                .replace("@@", "<tool_result>headline 1\nheadline 2</tool_result>")
        }),
    ];
    for (what, f) in mistakes {
        let caught = rendered.iter().filter(|(_, p)| f(p) != *p).count();
        assert!(caught > 0, "mistake '{what}' changes no fixture prompt: the gate cannot see it");
    }
}
...
fn thinking_resolution_contract() {
    ...
    assert_eq!(r(json!({"reasoning_effort": "none", "enable_thinking": true})), (true, 75));
...
fn calls_json(calls: &[parse::ParsedCall]) -> Value {
    Value::Array(
        calls.iter().map(|c| serde_json::json!({"name": c.name, "arguments": c.arguments})).collect(),
    )
}
...
fn parse_fixtures_match_python() {
    ...
    for c in cases(&fx) {
        ...
        let full = if text.ends_with(encoding::EOS) { text.to_string() } else { format!("{text}{}", encoding::EOS) };

        // strict: same verdict, same result
        match (parse::parse_strict(&full, thinking), c.get("strict")) {
            (Ok(m), Some(want)) => {
                assert_eq!(m.content, want["content"].as_str().unwrap(), "{name}: strict content");
                assert_eq!(m.reasoning_content, want["reasoning_content"].as_str().unwrap(), "{name}: reasoning");
                let got: Vec<Value> = m
                    .tool_calls
                    .iter()
                    .map(|t| {
                    ...
                    .collect();
                assert_eq!(Value::Array(got), want["tool_calls"], "{name}: strict calls");
            }
            (Err(e), None) => assert!(c.get("strict_error").is_some(), "{name}: {e}"),
            (Ok(m), None) => panic!("{name}: Python strict raised {} but Rust parsed {m:?}", c["strict_error"]),
            (Err(e), Some(_)) => panic!("{name}: Rust strict failed ({e}) where Python parsed"),
        }

        assert_eq!(calls_json(&parse::parse_tolerant(&full)), c["tolerant"], "{name}: tolerant");
        ...
        for chunk in [usize::MAX, 1, 3, 7] {
            ...
            let s = &c["server"];
            assert_eq!(router.reasoning, s["reasoning"].as_str().unwrap(), "{name}/{chunk}: reasoning");
            assert_eq!(router.content, s["content"].as_str().unwrap(), "{name}/{chunk}: content");
...
fn parse_controls_fail_where_they_must() {
    ...
    let one = by("one_call");
    let mutated = one["text"].as_str().unwrap().replace("｜DSML｜ ", "｜DSML｜");
    let mut router = parse::OutputRouter::new(false, vec![], true);
    router.feed(&mutated);
    router.finish();
    assert!(router.parse_tool_calls(false).is_empty(), "V4 spelling was parsed as a V4.1 call");
...
fn stop_string_and_marker_holdback() {
    ...
    assert_eq!(out, "reasonans", "the partial marker must be held back, never streamed");

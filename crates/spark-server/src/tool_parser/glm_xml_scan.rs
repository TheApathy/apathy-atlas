...
///
/// all-models port: narrower than the GLM branch so that no body another
/// parser can emit is reclassified. A bare-text head that is not a tool name
/// (prose inside an envelope) and an empty envelope (`<tool_call></tool_call>`,
/// whitespace allowed) keep their legacy handling; only a bare tool name or a
/// head-less body starting with a GLM arg marker (or a proper prefix of one
/// while streaming) is native.

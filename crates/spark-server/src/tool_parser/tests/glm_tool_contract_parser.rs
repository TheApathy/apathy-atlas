...
//! coercion and detector. Event assembly below is a transport observer, not a
//! replacement wire parser. No GPU, checkpoint or process access.
...
fn typed(mut calls: Vec<ToolCall>) -> Vec<ToolCall> {
    // Same production schema pass used by blocking/buffered handlers. Registry
    // tests independently require the selected parser to request this pass.
    ...
    for event in events {
        match event {
            ...
            DetectorOutput::ToolCallArgsFragment { fragment, idx } => {
                pending
                    .get_mut(&idx)
                    .expect("fragment without header")
                    .function
                    .arguments
                    .push_str(&fragment);
            }
...
fn run(chunks: &[&str], buffered: bool) -> (String, Vec<ToolCall>) {
    let mut detector = StreamingToolDetector::new_with_tools(tools());
    // Set request-local test state; never mutate global process environment.
...
fn glm_tool_contract_all_utf8_splits_and_character_bursts_match_blocking() {
    ...
    for buffered in [false, true] {
        for split in (0..=input.len()).filter(|&i| input.is_char_boundary(i)) {
            let (content, calls) = run(&[&input[..split], &input[split..]], buffered);
            assert!(content.is_empty(), "GLM markup leaked at split {split}");
            assert_eq!(calls.len(), 1, "split {split}, buffered={buffered}");
            assert_call(&calls[0], "inspect", &expected());
        }
        ...
        let (_, calls) = run(&chunks, buffered);
        assert_eq!(calls.len(), 1);
        assert_call(&calls[0], "inspect", &expected());
    }
}
...
fn glm_tool_contract_zero_args_and_repeated_name_calls_keep_order_and_ids() {
    ...
    for buffered in [false, true] {
        let (content, calls) = run(&[input], buffered);
...
fn glm_tool_contract_value_region_owns_unrelated_xml_and_tool_close_text() {
    ...
    for buffered in [false, true] {
        for split in (0..=input.len()).filter(|&i| input.is_char_boundary(i)) {
            let (_, calls) = run(&[&input[..split], &input[split..]], buffered);

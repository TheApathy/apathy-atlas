...
fn message(role: Role, content: Vec<ContentPart>) -> Message {
    Message { role, content, tool_calls: Vec::new(), tool_call_id: None, name: None,
        reasoning: None, tool_error: false }
}

fn text(parts: &mut Vec<ContentPart>, value: String) {
    if value.is_empty() { return; }
    if let Some(ContentPart::Text(prior)) = parts.last_mut() { prior.push_str(&value); }
    else { parts.push(ContentPart::Text(value)); }
}
...
fn image(source: super::types::ImageSourceBlock) -> ContentPart {
    ...
    let uri = source.maybe_get_image_uri().expect("validated Anthropic image source");
    ContentPart::Image(ImageSource { data: ImageData::from_uri(uri) })
}
...
fn tool_parts(content: Option<ToolResultContent>) -> Vec<ContentPart> {
    match content {
        ...
        Some(content) if !content.contains_image() => {
            ...
            if value.is_empty() { Vec::new() } else { vec![ContentPart::Text(value)] }
        }
...
pub(super) fn lower(role: Role, blocks: Vec<ContentBlock>) -> Vec<Message> {
    ...
    for block in blocks {
        match block {
            ...
            ContentBlock::Thinking { thinking } => {
                if let Some(value) = thinking && !value.is_empty() { reasoning.push(value); }
            }
            ContentBlock::ToolResult { tool_use_id, content, is_error } => {
                if role != Role::Assistant {
                    if !parts.is_empty() { messages.push(message(Role::User, std::mem::take(&mut parts))); }
    ...
    if role == Role::Assistant {
        ...
        if !reasoning.is_empty() { result.reasoning = Some(Reasoning { text: reasoning.join("\n") }); }
        messages.push(result);

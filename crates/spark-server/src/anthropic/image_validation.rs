...
use super::types::{
    AnthropicContent, ContentBlock, ImageSourceBlock, MessagesRequest, SystemContent,
    ToolResultContent,
};
...
impl ImageSourceBlock {
    fn validate(&self) -> Result<(), &'static str> {
        match self.source_type.as_str() {
            "base64"
                if self.data.as_ref().is_some_and(|s| !s.trim().is_empty())
                    && matches!(
            ...
            "url"
                if self
                    .url
                    .as_ref()
                    .is_some_and(|s| s.starts_with("data:image/") && s.contains(";base64,")) =>
            {
                Ok(())
            }
            _ => Err(

...
pub(super) fn validate_fp8_config(raw: &Value) -> Result<bool> {
    ...
    for (key, expected) in [
        ...
        if config.get(key).and_then(Value::as_str) != Some(expected) {
            bail!("glm5_next quantization_config.{key} must be {expected}");
        }
    }
    let block = config
        .get("weight_block_size")
        .and_then(Value::as_array)
        .context("glm5_next quantization_config.weight_block_size must be an array")?;
    if block.len() != 2 || block.iter().any(|value| value.as_u64() != Some(128)) {
        bail!("glm5_next quantization_config.weight_block_size must be [128, 128]");
    }
    let exclusions = config
        .get("modules_to_not_convert")
        .and_then(Value::as_array)
        .context("glm5_next quantization_config.modules_to_not_convert must be an array")?;
    if exclusions.is_empty()
        || exclusions
            .iter()
            .any(|value| value.as_str().is_none_or(str::is_empty))
    {
        bail!("glm5_next modules_to_not_convert must contain nonempty strings");
    }
    Ok(true)
}

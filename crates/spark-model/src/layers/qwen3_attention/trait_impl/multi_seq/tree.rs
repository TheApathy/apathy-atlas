...
impl Qwen3AttentionLayer {
    ...
    /// (MLA / mHC route their cache writes through fused paths that
    /// cannot be range-split); the caller falls back to per-row
    /// sequential decode for this layer.
    ...
    pub(in crate::layers::qwen3_attention) fn decode_multi_seq_tree_inner(
        ...
        // MLA (absorbed cache write inside ms_mla_decode) and mHC (its own
        // phase ordering) cannot split the cache write — per-row fallback.

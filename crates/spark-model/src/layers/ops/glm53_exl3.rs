...
#[path = "glm53_exl3_verify_regular.rs"]
mod regular_verify;
...
impl Glm53Exl3GemmKernel {
    ...
    pub fn load_row_exact(gpu: &dyn GpuBackend, plan: &Glm53Exl3GemmPlan) -> Result<Self> {
        ...
        let regular = regular_verify::selected(
            std::env::var_os("ATLAS_GLM53_EXL3_VERIFY_REGULAR").as_deref(),
            super::glm53_exact_verify_active(),
            super::glm53_exact_wide_prefill_active(),
            plan.rows,
        )
        .map_err(anyhow::Error::msg)?;
        // Diagnostic opt-in preserves geometry, grid and arithmetic plan. It
        // requires independent operator and full-model proof before use.
        let symbol = if regular {
            plan.symbol()
        } else {
            format!("glm53_exl3_rowexact_k{}_s{}", plan.bits, plan.shape_index)
        };

...
pub(super) fn sha256(bytes: &[u8]) -> [u8; 32] {
    ...
    for chunk in &mut chunks {
        compress(
            &mut state,
            chunk.try_into().expect("SHA256 chunk is exactly 64 bytes"),
        );
    }
    ...
    tail[padded_len - 8..padded_len].copy_from_slice(&(bit_len as u64).to_be_bytes());
    for chunk in tail[..padded_len].chunks_exact(64) {
        compress(
            &mut state,
            chunk.try_into().expect("SHA256 tail is exactly 64 bytes"),
        );
    }
...
fn compress(state: &mut [u32; 8], chunk: &[u8; 64]) {
    ...
    for (word, input) in schedule[..16].iter_mut().zip(chunk.chunks_exact(4)) {
        *word = u32::from_be_bytes(input.try_into().expect("SHA256 word is exactly four bytes"));
    }
    for index in 16..64 {

...
impl StagedLogits {
    pub fn read(request: &VerifyRequest, io: &mut dyn LogitsIo) -> Result<Self> {
        ...
        bytes
            .try_reserve_exact(request.total_bytes)
            .context("GLM policy logits allocation failed")?;
        bytes.resize(request.total_bytes, 0);
        let copied = io
            .copy_logits(&mut bytes)
...
pub fn run_verify_policy_transaction(
    ...
    let logits = match StagedLogits::read(request, source) {
        Ok(logits) => logits,
    ...
    let selection = select(request, &logits, policy);

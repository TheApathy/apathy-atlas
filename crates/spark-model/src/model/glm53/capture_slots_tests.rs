...
fn widened() -> GgmlIqBuffer {
    GgmlIqBuffer {
        ptr: DevicePtr(0x9000_0000),
        bytes: GLM53_CAPTURE_SLOT_BYTES as usize,
    }
}

#[test]
fn slots_tile_the_reserved_region_exactly() {
    let s = slots();
    let mut spans = Vec::new();
    for slot in 0..5 {
        let buffer = s.slot(slot).unwrap();
        assert_eq!(buffer.bytes as u64, GLM53_CAPTURE_SLOT_BYTES);
...
fn a_layer_written_into_the_wrong_slot_is_refused() {
    ...
    let s = slots();
    // Layer 14 belongs in slot 1.
    assert!(s.capture(&gpu, 14, 1, widened(), 0).is_ok());
    for wrong in [0, 2, 3, 4] {
        let error = s
            .capture(&gpu, 14, wrong, widened(), 0)
...
#[test]
fn a_mis_sized_or_null_source_is_refused() {
    let gpu = MockGpuBackend::new();
    let s = slots();
    let short = GgmlIqBuffer {
        bytes: GLM53_CAPTURE_SLOT_BYTES as usize - 2,
        ..widened()
    };
    assert!(s.capture(&gpu, 5, 0, short, 0).is_err());
    let null = GgmlIqBuffer {
        ptr: DevicePtr(0),
        ..widened()
    };
    assert!(s.capture(&gpu, 5, 0, null, 0).is_err());
}

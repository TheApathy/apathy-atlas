...
use crate::layers::ops::{
    ...
    Glm53Exl3RoutePolicy, NativeRowsSetting, glm53_layer_major_prefill_active,
};
...
impl Glm53Exl3Model {
    ...
    fn new_inner(
        ...
        let native_rows = NativeRowsSetting::parse_with_batched(
            std::env::var_os("ATLAS_GLM53_NATIVE_ROW_PREPARED").as_deref(),
            std::env::var_os("ATLAS_GLM53_NATIVE_ROW_BATCHED").as_deref(),
        )
        .map_err(anyhow::Error::msg)?;
        native_rows
            .validate_exact_mode(std::env::var_os("ATLAS_GLM53_EXACT_VERIFY").as_deref())
            .map_err(anyhow::Error::msg)?;
        let route_policy = Glm53Exl3RoutePolicy::parse_with_verify_group(
...
mod vision_splice_tests {
    ...
    #[test]
    fn policy_argmax_is_distinct_last_index_tie_and_nan_rejecting() {
        assert!(KDA_CUDA.contains("atlas_glm53_policy_argmax_bf16_rows("));
        assert!(KDA_CUDA.contains("candidate_index > best_index"));
        assert!(KDA_CUDA.contains("index == excluded_a || index == excluded_b"));
        assert!(KDA_CUDA.contains("shared_nan[threadIdx.x] |= shared_nan[threadIdx.x + stride]"));
        assert!(KDA_CUDA.contains("shared_nan[0] == 0U ? shared_values[0] : NAN"));
    }

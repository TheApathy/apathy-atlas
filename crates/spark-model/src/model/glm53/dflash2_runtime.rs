...
mod proposal;
#[path = "dflash2_readback.rs"]
pub(in crate::model::glm53) mod readback;
...
impl Glm53Dflash2Runtime {
    ...
    pub fn load(gpu: &dyn GpuBackend, root: &Path, lm_head: &Glm53Exl3Linear) -> Result<Self> {
        ...
        let readback = ProposalReadback::parse(
            std::env::var_os("ATLAS_GLM53_DFLASH2_COALESCED_READBACK").as_deref(),
        )?;
        if readback.is_coalesced() {
            tracing::info!(
                schema = "atlas.glm53.dflash2.coalesced_readback.v1",
                "GLM DFlash2 coalesced proposal readback engaged"
            );
        }
        ...
        let capture_input = gpu.alloc(QUERY_TOKENS as usize * 5 * HIDDEN as usize * 2)?;
    ...
    pub fn propose(&self, target: &Glm53Exl3Model, anchor: u32, stream: u64) -> Result<[u32; 7]> {
        ...
        self.readback.read(target.gpu(), path, status, stream)
    }
    ...
    pub fn propose_graph_probe(
        ...
        let replay_result = gpu
            ...
            .and_then(|()| self.readback.read(gpu, path, status, stream));

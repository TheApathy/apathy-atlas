...
impl LogitsIo for PolicyLogits<'_> {
    ...
    fn copy_argmax(&mut self, destination: &mut [u32], excluded: [u32; 2]) -> Result<usize> {
            ...
            destination.len() == self.rows,
            "GLM compact policy destination row count changed"
        );
        self.model.copy_policy_argmax(
            self.logits,
            self.rows,
            destination,
            excluded,
        ...
        if !REPORTED.swap(true, std::sync::atomic::Ordering::Relaxed) {
            tracing::info!(
                schema = "atlas.glm53.policy_argmax.v1",
                rows = self.rows,
                readback_bytes = DFLASH2_ARGMAX_BYTES,
                "GLM_POLICY_DEVICE_ARGMAX_ENGAGED"
            );
        }
        Ok(self.rows)
    }
    ...
    pub fn stage(
        ...
        let staged = timing.measure(Phase::WideStage, || -> Result<DevicePtr> {
            let (logits, at, count) = if exact_verify {
                with_glm53_native_rows(model.native_rows, || {

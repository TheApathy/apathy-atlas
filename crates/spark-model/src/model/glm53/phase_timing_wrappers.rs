...
impl LogitsIo for TimedLogits<'_, '_> {
    ...
    fn copy_argmax(&mut self, destination: &mut [u32], excluded: [u32; 2]) -> Result<usize> {
        self.timing.measure(Phase::Readback, || {
            self.inner.copy_argmax(destination, excluded)
        })
    }
}
...
impl VerifyPolicy for TimedPolicy<'_, '_> {
    ...
    fn compact_argmax_exclusions(&self) -> Option<[u32; 2]> {
        self.inner.compact_argmax_exclusions()
    }
    ...
    fn pick_argmax(&mut self, row: usize, token: u32) -> Result<u32> {
        self.timing
            .measure(Phase::PolicyPick, || self.inner.pick_argmax(row, token))
    }

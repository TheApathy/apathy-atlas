...
impl Glm53DsaAttentionKernels {
    ...
    /// Compute only the row-independent DSA inputs once for a wide prompt
    /// chunk. Exact-wide verification retains its row-exact projection path;
    ...
    fn precompute_wide_exl3_rows(
        ...
        let max_rows = if layer_major {
            ...
            "GLM EXL3 DSA precompute requires a wide prefill scope"
        );
        if exact_wide {
            ensure!(

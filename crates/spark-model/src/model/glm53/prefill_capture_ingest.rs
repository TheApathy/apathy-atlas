...
use super::prefill_capture_plan::{
    BoundCapturePlan, CaptureTransfer, CopyStep, DeviceSpan, GatherStep, ProjectionStep,
};
...
impl CaptureReceipt {
    ...
    pub(crate) fn ingest(
        ...
        let transfer_rows = self.bound.transfer_rows();
        let slices = (0..plan.rows())
            .step_by(transfer_rows as usize)
            .map(|first| {
                self.bound
                    .slice(first, transfer_rows.min(plan.rows() - first))
            })
        ...
        let execution = (|| -> Result<()> {
            for slice in slices {
                match slice.transfer {
                    CaptureTransfer::Copies(copies) => {
                        for copy in copies {
                            io.copy(copy, stream)?;
                        }
                    }
                    CaptureTransfer::Gather(gather) => io.gather(gather, stream)?,
                }

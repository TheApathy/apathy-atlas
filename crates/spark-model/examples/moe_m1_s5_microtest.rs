...
//! Bit-exactness + perf oracle for the M=1 **serial-decode v5** cp.async MoE
//! expert kernels (`moe_expert_gate_up_shared_v5` /
//! `moe_expert_silu_down_shared_v5`) against the shipping serial pair
//! (`moe_expert_gate_up_shared` / `moe_expert_silu_down_shared`), at exact
//! Laguna-S-2.1 single-token decode shapes (hidden K=3072, expert inter
//! N=1024, top_k=10 routed experts, NVFP4 non-transposed + E4M3 group-16
//! scales + per-tensor scale2, 256-wide pointer tables).
//!
//! v5 is required to be BYTE-IDENTICAL to the serial kernels (same lane
//! partition — k16 = lane + 32·i ascending — same uint64 nibble decode and
//! FMA order, same SWIGLU clamp; only the load path changes: whole 8-row
//! weight slices cp.async-staged in 1024-elem commit groups, down covers 16
//! rows/block), so the gate here is exact u16 equality of every output
//! buffer. Runs both the populated-shared and the NULL-shared (production
//! Laguna) variants.
//!
//! Usage:
//!   cargo run --release -p spark-model --example moe_m1_s5_microtest \
//!       -- [top_k] [pool] [seed-hex]
//! Defaults: 10 256 0x9E3
...
fn main() -> Result<()> {
    ...
    let top_k: usize = args.get(1).map_or(10, |s| s.parse().unwrap());
    let pool: usize = args.get(2).map_or(256, |s| s.parse().unwrap());
    let seed: u64 = args.get(3).map_or(0x9E3, |s| {
        u64::from_str_radix(s.trim_start_matches("0x"), 16).unwrap_or(0x9E3)
    });
    assert!(pool <= NUM_EXPERT_SLOTS && top_k <= pool);

    println!(
        "=== M=1 serial-decode v5 microtest: top_k={top_k} pool={pool} \
    ...
    // top_k distinct experts (single token — no cross-token dedup exists at
    // M=1). NUM_IDX_SETS index sets: set 0 drives the bit-exactness gate; the
    // perf legs cycle through all of them so consecutive launches touch
    // different expert weights (production: every layer streams fresh
    // weights — a single hot set would be L2-resident and overstate GB/s).
    const NUM_IDX_SETS: usize = 16;
    let mut idx_sets_host: Vec<Vec<u32>> = Vec::with_capacity(NUM_IDX_SETS);
    for _ in 0..NUM_IDX_SETS {
        let mut indices: Vec<u32> = Vec::with_capacity(top_k);
        ...
        idx_sets_host.push(indices);
    }
    let indices = idx_sets_host[0].clone();
    println!("routing (set 0): {indices:?}");
    ...
    // null_shared=true reproduces production Laguna (no shared expert →
    // NULL pointers, zero-filled shared rows).
    let launch_gate_up =
        |name: &str, null_shared: bool, idx: DevicePtr, stream: u64| -> Result<()> {
            let h = gpu.kernel("moe_shared_expert_fused", name)?;
            ...
            KernelLaunch::new(gpu, h)
                .grid([(INTER as u32).div_ceil(8), top_k as u32 + 1, 2])
                .block([128, 1, 1])
                .arg_ptr(a_ptr)
                .arg_ptr(gate_tbl.0)
                .arg_ptr(gate_tbl.1)
    ...
    // v1 covers 8 rows/block (grid ceil(N/8)); v5 covers 16 (grid ceil(N/16),
    // 2 pipelined cp.async row tiles). Both take K*4 dynamic smem (s_act).
    let launch_down = |name: &str, null_shared: bool, idx: DevicePtr, stream: u64| -> Result<()> {
        let h = gpu.kernel("moe_shared_expert_fused", name)?;
        ...
        KernelLaunch::new(gpu, h)
            .grid([
                (HIDDEN as u32).div_ceil(rows_per_block),
                top_k as u32 + 1,
                1,
            ])
            .block([128, 1, 1])
            .shared_mem(INTER as u32 * 4)
            .arg_ptr(gate_out)
            .arg_ptr(up_out)
            .arg_ptr(down_tbl.0)
            .arg_ptr(down_tbl.1)
    ...
    // ── bit-exactness: serial chain vs v5 chain, byte-for-byte ──
    let mut fail = false;
    for null_shared in [false, true] {
        ...
        for chain in ["serial", "v5"] {
            // poison every output buffer so unwritten rows can't fake a match
            fill_garbage(gate_out, top_k * INTER)?;
            fill_garbage(up_out, top_k * INTER)?;
            fill_garbage(down_out, top_k * HIDDEN)?;
            fill_garbage(sh_gate_out, INTER)?;
            fill_garbage(sh_up_out, INTER)?;
            fill_garbage(sh_down_out, HIDDEN)?;
            gpu.synchronize(stream)?;
            if chain == "v5" {
                launch_gate_up("moe_expert_gate_up_shared_v5", null_shared, idx_ptr, stream)?;
                launch_down(
                    "moe_expert_silu_down_shared_v5",
                    null_shared,
                    idx_ptr,
                    stream,
                )?;
            } else {
                launch_gate_up("moe_expert_gate_up_shared", null_shared, idx_ptr, stream)?;
                launch_down("moe_expert_silu_down_shared", null_shared, idx_ptr, stream)?;
            }
            gpu.synchronize(stream)?;
            captured.push(vec![
    ...
    // ── perf: event-timed, per phase, serial vs v5, production null-shared ──
    ...
    // production Laguna (null-shared): the kernels read exactly top_k routed
    // experts' weights (packed + scales), once each.
    let gu_bytes = top_k * 2 * (INTER * HIDDEN / 2 + INTER * HIDDEN / GROUP);
    let dn_bytes = top_k * (HIDDEN * INTER / 2 + HIDDEN * INTER / GROUP);
    println!(
        "weight read (null-shared): gate_up {:.1} MB + down {:.1} MB = {:.1} MB/layer",
        gu_bytes as f64 / 1e6,
        dn_bytes as f64 / 1e6,
        (gu_bytes + dn_bytes) as f64 / 1e6
    );

    // Each timed launch rotates to the next index set so consecutive
    // iterations stream different expert weights (no L2 residency).
    ...
    let legs: Vec<(&str, usize, L)> = vec![
        (
            "gate_up serial",
            gu_bytes,
            Box::new(|s| launch_gate_up("moe_expert_gate_up_shared", true, next_idx(), s)),
        ),
        (
            "gate_up v5",
            gu_bytes,
            Box::new(|s| launch_gate_up("moe_expert_gate_up_shared_v5", true, next_idx(), s)),
        ),
        (
            "down serial",
            dn_bytes,
            Box::new(|s| launch_down("moe_expert_silu_down_shared", true, next_idx(), s)),
        ),
        (
            "down v5",
            dn_bytes,
            Box::new(|s| launch_down("moe_expert_silu_down_shared_v5", true, next_idx(), s)),
        ),
    ];
    ...
    let old = ms["gate_up serial"] + ms["down serial"];
    let new = ms["gate_up v5"] + ms["down v5"];
    let total = (gu_bytes + dn_bytes) as f64;
    println!(
        "chain serial: {old:.4} ms/layer ({:.0} GB/s) -> chain v5: {new:.4} ms/layer \
         ({:.0} GB/s)  [{:+.1}%]  (48 layers: {:.2} -> {:.2} ms/token FFN)",
        total / (old / 1e3) / 1e9,
        total / (new / 1e3) / 1e9,
        (old / new - 1.0) * 100.0,
        old * 48.0,
    ...
    if fail {
        eprintln!("RESULT: FAIL (v5 not bit-exact vs serial)");
        std::process::exit(1);
    }
    println!("RESULT: PASS");
    Ok(())

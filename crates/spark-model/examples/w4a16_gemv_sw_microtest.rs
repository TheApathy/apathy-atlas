...
//! Losslessness oracle for the single-warp-per-output decode GEMVs
//! (`w4a16_gemv_sw`, `w4a16_gemv_dual_sw`, `w4a16_gemv_silu_input_sw`) against
//! their 64-thread / 2-warp + smem-cross-warp-reduce bases.
//!
//! Ported from upstream `crates/spark-model/examples/w4a16_gemv_sw_microtest.rs`
//! (`7761b965`), with this tree's shapes and one shape removed — see
//! `N%4 != 0` below.
//!
//! Unlike a prefill BF16-TC oracle (which only requires cosine ≈ 1.0 because
//! the K-tile reassociation differs), these variants are engineered to be
//! BYTE-IDENTICAL: each SW lane reproduces the two base lanes `lane` and
//! `lane + 32` with the same K-chunk association, reduces each in the same
//! shuffle tree, and adds them in the same order as the base kernel's
//! `smem[0] + smem[1]`. **The PASS bar is `bit_id == 100%` on every shape.**
//! Anything less means the accumulation order diverged → lossy → STOP, do not
//! ship, and do not "accept" it on a cosine argument.
//!
...
fn main() -> Result<()> {
    ...
    // Decode GEMV shapes for Qwen3.8-27B (M=1), from the served config:
    // hidden 5120, intermediate 17408, 24 q-heads × 256, 4 kv-heads × 256,
    // GDN 16 k-heads × 128 + 48 v-heads × 128 → qkvz 16384, value_dim 6144.
    //
    // `N % 4 != 0` is DELIBERATELY ABSENT. Upstream tests it because upstream's
    // base kernels defer their `n >= N` return until after the smem barrier;
    // ours return before it, so on an N%4!=0 tail the base reads an
    // uninitialized `smem[local_out*2 + 1]` and the SW kernel — which has no
    // barrier and no smem — would be the *correct* one. That comparison would
    // report a mismatch caused by the base, not by the port. Every N this tree
    // actually launches is a multiple of 64, so the tail is unreachable; see
    // UPSTREAM-PORT.md for why the base-kernel fix was left out of scope.
    let shapes: &[(&str, usize, usize)] = &[
        ("ffn gate/up", 17408, 5120),
        ("ffn down   ", 5120, 17408),
        ("gdn qkvz   ", 16384, 5120),
        ("gdn out_prj", 5120, 6144),
        ("attn q_proj", 6144, 5120),
        ("attn kv    ", 1024, 5120),

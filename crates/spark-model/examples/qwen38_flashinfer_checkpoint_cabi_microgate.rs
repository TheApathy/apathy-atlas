...
//! Real-checkpoint parity gate for the isolated FlashInfer SM121 C ABI.
//!
...
//! seam.  Instead, a bounded safetensors reader seeks only one explicitly named
//! FFN projection's packed bytes, E4M3 scales, and two global scalars.
//!
//! CPU-only checkpoint admission (does not initialize CUDA):
//!
//! ```text
//! ATLAS_FI_CHECKPOINT_DIR=/path/to/Qwen3.8-27B-NVFP4 \
//! ATLAS_FI_LAYER=0 ATLAS_FI_PROJECTION=gate \
//! ATLAS_FI_METADATA_ONLY=1 \
//! cargo run -p spark-model --features cuda \
//!   --example qwen38_flashinfer_checkpoint_cabi_microgate
//! ```
//!
...
impl Projection {
    fn parse() -> Result<Self> {
        match required_env("ATLAS_FI_PROJECTION")?.as_str() {
            ...
            "up" => Ok(Self::Up),
            "down" => Ok(Self::Down),
            value => bail!("ATLAS_FI_PROJECTION must be gate, up, or down; got {value}"),
        }
    }
    ...
    fn shape(self) -> (usize, usize) {
        match self {
            Self::Gate | Self::Up => (17_408, 5_120),
            Self::Down => (5_120, 17_408),
        }
    }
}

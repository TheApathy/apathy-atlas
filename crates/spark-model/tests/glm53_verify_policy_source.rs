...
fn policy_transaction_is_integrated_not_just_an_unregistered_test_helper() {
    ...
    // The model cannot simultaneously borrow SequenceState while the real
    // policy adapter borrows ActiveSeq. Publication belongs to the scheduler
    // after dropping that adapter, not a fabricated second host sequence.
    let driver = include_str!("../../spark-server/src/scheduler/glm53_policy_driver.rs");
    assert!(
        driver.contains("receipt.publish("),
        "host prefix publication must consume an actual committed receipt"
    );
}

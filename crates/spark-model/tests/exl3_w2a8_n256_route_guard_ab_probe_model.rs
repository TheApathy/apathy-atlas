...
//! Source contracts for the immutable N256 route-guard composed W2A8 A/B probe.

use std::fs;
use std::path::PathBuf;

const HARNESS: &str = "kernels/gb10/experiments/exl3_w2a8_composed_n128_n256_probe.cu";
const BUILDER: &str = "scripts/check-exl3-prefill-w2a8-n256-route-guard-ab-probe-build.sh";
...
fn source(relative: &str) -> String {
    fs::read_to_string(workspace().join(relative))
        .unwrap_or_else(|error| panic!("missing N256 route-guard A/B input {relative}: {error}"))
}
...
#[test]
fn candidate_is_one_default_off_boolean_switch_in_only_the_n256_component() {
    ...
    assert_eq!(
        component
            .matches("#if W2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE")
            .count(),
        1
    );
}
...
#[test]
fn builder_freezes_packed_e4m3_and_changes_only_the_route_guard_selector() {
    let build = source(BUILDER);
    let threshold = marked(&build, "N256 route guard A/B threshold", "#");
    ...
    let inputs = marked(&build, "N256 route guard A/B immutable inputs", "#");
    for dependency in [
        ...
        "KERNEL.toml",
        "check-exl3-prefill-w2a8-n256-route-guard-ab-probe-build.sh",
    ] {
        assert!(
    ...
    let variants = marked(&build, "N256 route guard two binary build", "#");
    for proof in [
        ...
        "only_ab_compile_factor=W2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE",
...
fn generated_runner_is_fail_closed_and_owns_the_only_speed_threshold() {
    let build = source(BUILDER);
    let runner = marked(&build, "N256 route guard generated A/B runner", "#");
    for proof in [

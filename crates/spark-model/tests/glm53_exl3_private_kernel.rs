...
#[test]
fn selected_direct_prefill_uses_inverse_pack_direct_fold_and_regular_combine() {
    if std::env::var_os("ATLAS_GLM53_EXL3_MOE_PREFILL_DIRECT_COMBINE").as_deref()
        != Some(OsStr::new("1"))
    {
        return;
    }
    for rows in [1024, 2048] {
        ...
        assert_eq!(
            ls[0].args.last(),
            Some(&fixture::Arg::ptr(f.buffers.route_private_f32))
        );
        assert_eq!(
            (&*ls[6].module, &*ls[6].name),
            (
                "glm53_exl3_moe_staged_private",
                "atlas_glm53_exl3_staged_combine_private"
            )
        );
        assert_eq!(ls[6].grid, [rows, 32, 1]);
        assert_eq!(ls[6].block, [32, 1, 1]);
        assert_eq!(ls[6].shared, 512);
        assert_eq!(ls[6].args.len(), 9);
        assert_eq!(ls[6].args[0], fixture::Arg::ptr(f.buffers.temp_state_g_f16));
        assert_eq!(ls[6].args[1], fixture::Arg::ptr(f.buffers.output_f32));
        assert_eq!(
            ls[6].args[5],
            fixture::Arg::ptr(f.buffers.route_private_f32)
        );
        assert_eq!(ls[6].args[7], fixture::Arg::word(rows));
        assert_eq!(ls[6].args[8], fixture::Arg::word(p.pairs));
        check_combine(&ls[7], p, &f, false);
        assert!(
            gpu.memsets()
                .iter()
                .all(|(ptr, _, _, _)| *ptr != f.buffers.output_f32.ptr.0)

...
//! Source contracts for the immutable N256-down double-buffer W2A8 A/B probe.

use std::fs;
use std::path::PathBuf;

const HARNESS: &str = "kernels/gb10/experiments/exl3_w2a8_composed_n128_n256_probe.cu";
const BUILDER: &str = "scripts/check-exl3-prefill-w2a8-n256-down-double-buffer-ab-probe-build.sh";
...
fn source(relative: &str) -> String {
    fs::read_to_string(workspace().join(relative)).unwrap_or_else(|error| {
        panic!("missing N256-down double-buffer A/B input {relative}: {error}")
    })
}
...
#[test]
fn candidate_is_one_default_off_boolean_switch_in_the_n256_down_component() {
    let component = source(N256);
    for contract in [
...
#[test]
fn builder_uses_a_receipt_bound_experiment_wrapper_not_the_production_wrapper() {
    let build = source(BUILDER);
    let wrapper = marked(&build, "generated experiment-local N256 wrapper", "#");
    for proof in [
        "n256_experiment_wrapper=",
        ...
        "W2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE must be receipt-bound",
        "W2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE must be receipt-bound",
    ...
    let variants = marked(&build, "N256 down double buffer two binary build", "#");
    assert!(variants.contains("\"$n256_experiment_wrapper\""));
    assert!(!variants.contains("\"$n256_down_wrapper\" \\\n        -o \"$binary\""));
}
...
#[test]
fn builder_freezes_promoted_factors_and_changes_only_the_n256_down_selector() {
    let build = source(BUILDER);
    let threshold = marked(&build, "N256 down double buffer A/B threshold", "#");
    ...
    let inputs = marked(&build, "N256 down double buffer A/B immutable inputs", "#");
    for dependency in [
        ...
        "check-exl3-prefill-w2a8-n256-down-double-buffer-sass.sh",
        "KERNEL.toml",
        "check-exl3-prefill-w2a8-n256-down-double-buffer-ab-probe-build.sh",
    ] {
        assert!(
    ...
    for proof in [
        ...
        "only_ab_compile_factor=W2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE",
        ...
        "overlap_qmma != 16",
        "different_cubins == 1",
        "unchanged_n128_sass=PASS",
        "changed_n256_sass=PASS",
    ] {
...
fn generated_runner_is_fail_closed_and_owns_the_only_speed_threshold() {
    let build = source(BUILDER);
    let runner = marked(&build, "N256 down double buffer generated A/B runner", "#");
    for proof in [
        ...
        "n256_down_double_buffer_speedup",
        "speedup < threshold",

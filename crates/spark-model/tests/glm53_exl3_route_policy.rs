...
#[test]
fn verifier_group_and_staged_k32_are_explicit_small_exact_decode_only_policies() {
    for (raw, width) in [("2", 2), ("4", 4), ("8", 8)] {
        let grouped = Glm53Exl3RoutePolicy::parse_with_verify_group(
            Some(OsStr::new("1")),
            None,
            Some(OsStr::new(raw)),
            Some(OsStr::new("1")),
        ...
        .unwrap();
        assert_eq!(grouped.verify_group(6, true, false), Some(width));
        assert_eq!(grouped.verify_group(1, true, false), None);
        assert_eq!(grouped.verify_group(6, false, false), None);
        assert_eq!(grouped.verify_group(6, true, true), None);

        let staged = grouped.with_verify_staged_k32(Some(OsStr::new("1")), Some(OsStr::new("1")));
        assert_eq!(staged.is_ok(), width == 8);
        if let Ok(staged) = staged {
            assert!(staged.verify_staged_k32_enabled());
            assert!(staged.verify_staged_k32(6, true, false));
            assert!(!staged.verify_staged_k32(1, true, false));
            assert!(!staged.verify_staged_k32(6, false, false));
            assert!(!staged.verify_staged_k32(6, true, true));
        }
    }
}

...
#[test]
fn compact_policy_callbacks_cross_timing_decorators_without_falling_back() {
    struct CompactSource(Option<[u32; 2]>);
    impl LogitsIo for CompactSource {
        fn copy_logits(&mut self, _out: &mut [u8]) -> Result<usize> {
            bail!("full logits fallback is forbidden")
        }
        fn copy_argmax(&mut self, out: &mut [u32], excluded: [u32; 2]) -> Result<usize> {
            self.0 = Some(excluded);
            out.copy_from_slice(&[7, 8]);
            Ok(out.len())
        }
    }

    struct CompactPolicy(Vec<(usize, u32)>);
    impl VerifyPolicy for CompactPolicy {
        fn checkpoint(&mut self) -> Result<()> {
            Ok(())
        }
        ...
        fn pick(&mut self, _row: usize, _logits: &[u8]) -> Result<u32> {
            bail!("full logits pick is forbidden")
        }
        fn pick_argmax(&mut self, row: usize, token: u32) -> Result<u32> {
            self.0.push((row, token));
            Ok(token)
        }
        fn advance(&mut self, _token: u32) -> Result<PolicyAdvance> {
    ...
    let clock = CounterClock::default();
    let setting = TimingSetting::parse(Some(OsStr::new("1"))).unwrap();
    let recorder = PhaseRecorder::new(setting, &clock);
    let mut source = CompactSource(None);
    let mut output = [0u32; 2];
    {
        let mut timed = TimedLogits::new(&mut source, &recorder);
        assert_eq!(timed.copy_argmax(&mut output, [1, 4]).unwrap(), 2);
    }
    assert_eq!(source.0, Some([1, 4]));
    assert_eq!(output, [7, 8]);

    let mut policy = CompactPolicy(Vec::new());
    {
        let mut timed = TimedPolicy::new(&mut policy, &recorder);
        assert_eq!(timed.compact_argmax_exclusions(), Some([1, 4]));
        assert_eq!(timed.pick_argmax(1, 8).unwrap(), 8);
    }
    assert_eq!(policy.0, vec![(1, 8)]);
    let receipt = recorder.receipt().unwrap();
    assert_eq!(receipt.phase(Phase::Readback).calls, 1);
    assert_eq!(receipt.phase(Phase::PolicyPick).calls, 1);
}

...
pub(super) fn digest(input: &[u8]) -> [u8; 32] {
    ...
    let minimum_len = input
        .len()
        .checked_add(9)
        .expect("test source minimum padded length fits usize");
    let padded_len = minimum_len
        .checked_add(63)
...
pub(super) fn matches(source: &str, canonical: &str) -> bool {
    hex(digest(source.as_bytes())) == canonical
}

#[test]
fn known_vectors_cover_sha256_padding_boundary() {
    assert_eq!(
        hex(digest(b"")),

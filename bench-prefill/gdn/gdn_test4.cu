...
int main(int argc, char** argv) {
  ...
  size_t KS = 130;
  size_t base4 = 2*C_*KS*2 + C_*C_*4 + C_*4*2 + pairs*2;
  base4 = (base4 + 15)/16*16;
  size_t smem_c4 = base4 + 3*C_*VD*4 + C_*VD*4;
  smem_c4 = (smem_c4 + 255)/256*256;
  printf("smem: ref=%zu cand3=%zu cand4=%zu (optin limit 101376)\n", smem_ref, smem_c3, smem_c4);
  cudaFuncSetAttribute(gdn_candidate4, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem_c4);
  ...
  auto run = [&](int which, float* dH, uint16_t* dO) {
    ...
    else if (which == 1) gdn_candidate3<<<dim3(NV,1), 512, smem_c3>>>(
        dH, q, k, v, dG, dG + NV, (__nv_bfloat16*)dO, 1, L, NK, NV, KD, VD, conv_dim, conv_dim, gb_stride);
    else gdn_candidate4<<<dim3(NV,1), 512, smem_c4>>>(
        dH, q, k, v, dG, dG + NV, (__nv_bfloat16*)dO, 1, L, NK, NV, KD, VD, conv_dim, conv_dim, gb_stride);
  };
  const char* nm[3] = {"reference","cand3","cand4"};
  for (int which = 0; which < 3; which++) {
    ...
    if (which) { /* compare this candidate against the reference below */ }
  }

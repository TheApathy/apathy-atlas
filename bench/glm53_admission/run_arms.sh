...
for arm in "$@"; do
  echo "=== arm $arm $(date +%T)"
  case $arm in
    prefill)        python3 $HERE/atlas_driver.py --bin "$BIN" --out "$ROOT/$arm" --prefill $P --env $ESC ;;
    prefill256)     python3 $HERE/atlas_driver.py --bin "$BIN" --out "$ROOT/$arm" --prefill $P --env $ESC --env $R256 ;;

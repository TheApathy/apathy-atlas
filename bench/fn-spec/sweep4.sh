...
# A load whose log is silent for STALL_S (240) seconds is abandoned (rc=8).
set -uo pipefail
NAME=$1; BIN=$2; ARMS=$3; REPS=${4:-5}
SRC=$(cd "$(dirname "$0")" && pwd)
OUT=/home/flocka/atlas/fn-spec-bench/runs/$NAME; mkdir -p "$OUT"
cp "$ARMS" "$OUT/arms.txt"; cp "$SRC/client.py" "$OUT/"; cp "${PROMPTS:-$SRC/prompts.json}" "$OUT/prompts.json"
...
PORT=8897; MEM_NEED_GB=${MEM_NEED_GB:-100}; FLOOR_GB=${FLOOR_GB:-8}
...
run_arm() {
  ...
  setsid env -i HOME=$HOME LANG=C.UTF-8 PATH=/usr/local/cuda-13.0/bin:/usr/bin:/bin \
    ...
    timeout -s INT -k 60 2400 "$BIN" serve --model-from-path $MODEL_DIR --model-name m --port $PORT \
      --kernel-target qwen3.8-flash-next --max-seq-len 4096 --max-prefill-tokens ${PREFILL_TOK:-2048} \
      --ssm-cache-slots ${SSM_SLOTS:-16} --kv-cache-dtype bf16 --qwen4-qsa --gpu-memory-utilization 0.85 \
  ...
  for i in $(seq 1 900); do
    ...
    # FN load can hang with a silent log (infra-audit's open bug): give up
    # on the arm after STALL_S seconds of silence instead of the full wait.
    AGE=$(( $(date +%s) - $(stat -c %Y "$RUN/server.log") ))
    [ "$AGE" -gt "${STALL_S:-240}" ] && { RC=8; echo "load stalled ${AGE}s" > "$RUN/stall.txt"; break; }
    sleep 1
  done

#!/usr/bin/env bash
# runK_32k_inputs: the runK prompt with attention2's core INPUT taps at L2/L20/L24 (all chunks; ~10 GB) so its
# gate can feed oracle inputs straight into the core (teacher-forced topk: index logic vs drift).
...
echo "$(date -u +%FT%TZ) dsv41-integrate runK_32k_inputs capture (core INPUT taps for attention2 teacher-forced gate) (python engine, K124 arena ~72GB, 32K prompt, ~20min) QUEUED pid=$$" >> "$Q"
...
export CONTAINER_NAME="dsv41-integrate-runKi-$$"
LOW=/tmp/claude-1000/-home-flocka-atlas/153d8f07-762d-4884-a8f5-8f7a1a13f1ce/scratchpad/runKi_low.txt
( until docker ps --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; do sleep 2; kill -0 $$ 2>/dev/null || exit 0; done
  echo "$(date -u +%FT%TZ) dsv41-integrate runKi window START (lock held, container running) pid=$$" >> "$Q"
  low=999999999
...
/home/flocka/atlas/dsv41-prefill-work/bench/in_image.sh /oracle/capture_ref.py \
  --out /oracle/ref/runK_32k_inputs --attn torch --tap-logits \
  --prompt-ids-json /oracle/prompts_32k.json --prompt-name code_32k \
  --max-prompt-tokens 32768 --max-seq 34816 --layers 2,20,24 --taps attn_x,qr,q,ring,wpos,win_lo,attn_sink,ckv,ik,cand_in,topk,n_c,attn_out
rc=$?
echo "$(date -u +%FT%TZ) dsv41-integrate runKi window END rc=$rc lowwater=$(cat $LOW 2>/dev/null)" >> "$Q"
echo "DONE runKi rc=$rc lowwater=$(cat $LOW 2>/dev/null)"

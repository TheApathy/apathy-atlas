#!/usr/bin/env bash
# runK_32k_kernel: the SELF-DIVERGENCE GAUGE for runK_32k (same capture, Python production attention kernel instead of torch). The first oracle where the sparse index truly PRUNES (32,768 real code tokens: top-512 of
...
echo "$(date -u +%FT%TZ) dsv41-integrate runK_32k_kernel gauge capture (python engine, K124 arena ~72GB, 32K prompt, ~20min) QUEUED pid=$$" >> "$Q"
...
export CONTAINER_NAME="dsv41-integrate-runKk-$$"
LOW=/tmp/claude-1000/-home-flocka-atlas/153d8f07-762d-4884-a8f5-8f7a1a13f1ce/scratchpad/runKk_low.txt
( until docker ps --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; do sleep 2; kill -0 $$ 2>/dev/null || exit 0; done
  echo "$(date -u +%FT%TZ) dsv41-integrate runKk window START (lock held, container running) pid=$$" >> "$Q"
  low=999999999
...
/home/flocka/atlas/dsv41-prefill-work/bench/in_image.sh /oracle/capture_ref.py \
  --out /oracle/ref/runK_32k_kernel --attn kernel --tap-logits --gen-tokens 16 \
...
rc=$?
echo "$(date -u +%FT%TZ) dsv41-integrate runKk window END rc=$rc lowwater=$(cat $LOW 2>/dev/null)" >> "$Q"
echo "DONE runKk rc=$rc lowwater=$(cat $LOW 2>/dev/null)"

#!/usr/bin/env bash
# runG: THE end-to-end oracle for the port (lead ruling): checkpoint numerics + SWA bounded replay ON.
...
echo "$(date -u +%FT%TZ) dsv41-integrate runG faithful+SWA-replay oracle capture (python engine, K124 arena ~72GB, ~15min) QUEUED pid=$$" >> "$Q"
...
export CONTAINER_NAME="dsv41-integrate-runG-$$"
( until docker ps --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; do sleep 2; kill -0 $$ 2>/dev/null || exit 0; done
  echo "$(date -u +%FT%TZ) dsv41-integrate runG window START (lock held, container running) pid=$$" >> "$Q" ) &
/home/flocka/atlas/dsv41-prefill-work/bench/in_image.sh /oracle/capture_ref.py \
  --out /oracle/ref/runG_replay --attn torch --tap-logits --gen-tokens 48 \
...
rc=$?
echo "$(date -u +%FT%TZ) dsv41-integrate runG window END rc=$rc" >> "$Q"
echo "DONE runG rc=$rc"

#!/usr/bin/env bash
# runI: the oracle's own distribution at greedy step 20 (prefill of prompt + its first 20 generated tokens), plus L24/L28 indexer taps in the replay.
...
echo "$(date -u +%FT%TZ) dsv41-integrate runI runG+20 near-tie probe (prefill of prompt+20 oracle tokens, L24/L28 replay taps) (python engine, K124 arena ~72GB, ~15min) QUEUED pid=$$" >> "$Q"
...
export CONTAINER_NAME="dsv41-integrate-runI-$$"
( until docker ps --format '{{.Names}}' | grep -qx "$CONTAINER_NAME"; do sleep 2; kill -0 $$ 2>/dev/null || exit 0; done
  echo "$(date -u +%FT%TZ) dsv41-integrate runI window START (lock held, container running) pid=$$" >> "$Q" ) &
/home/flocka/atlas/dsv41-prefill-work/bench/in_image.sh /oracle/capture_ref.py \
  --out /oracle/ref/runI_plus20 --attn torch --tap-logits --gen-tokens 4 \
  --prompt-ids-json /oracle/runG_plus20.json --prompt-name runG_plus20 \
  --max-prompt-tokens 1044 --max-seq 8192 --layers 24,28
rc=$?
echo "$(date -u +%FT%TZ) dsv41-integrate runI window END rc=$rc" >> "$Q"
echo "DONE runI rc=$rc"

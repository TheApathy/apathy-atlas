...
import b62_prefill_ab_process as process_identity
import b62_prefill_ab_validate as validate
from test_b62_prefill_ab import receipt_line, response_for, server_log
...
class HostileLogTests(unittest.TestCase):
    ...
    def test_receipt_and_route_mutations_reject(self) -> None:
        valid = server_log("attention")
        mutations = (
            valid.replace("engaged_layers=12", "engaged_layers=11", 1),
            valid.replace("M=38", "M=2013", 1),
            valid.replace("attention_selector=1", "attention_selector=0", 1),
            valid.replace("serialized_fallback=false", "serialized_fallback=true", 1),
            valid + receipt_line("attention", 2013) + "\n",
        ...
        for text in mutations:
            with self.subTest():
                self.check_rejects(text, "attention")

    def test_control_and_cross_family_receipts_reject(self) -> None:
        self.check_rejects(
            server_log("control") + receipt_line("attention", 38) + "\n", "control"
        )
        self.check_rejects(
            server_log("ssm").replace("family=ssm", "family=attention", 1), "ssm"
        )

    def test_duplicate_receipt_field_rejects(self) -> None:
        text = server_log("attention").replace("value=1", "value=1 value=1", 1)
        self.check_rejects(text, "attention")
...
class StaticContractTests(unittest.TestCase):
    def test_arm_environment_and_launch_are_isolated(self) -> None:
        self.assertEqual(contract.ARM_ORDER, ("control", "attention", "ssm"))
        for arm, expected in contract.ARM_SELECTORS.items():
            environment = process_identity.arm_environment(arm)
            self.assertEqual(
                (
                    environment["ATLAS_QWEN4_ATTN_PREFILL_BATCH"],
                    environment["ATLAS_QWEN4_SSM_PREFILL_BATCH"],
                ),
                expected,
            )
            self.assertEqual(environment["ATLAS_PLE_CACHE_MB"], "0")
    ...
    def test_failure_schema_has_no_performance_fields(self) -> None:
        source = (HERE / "b62_prefill_ab.py").read_text()
        failure = source.split('"schema": "atlas-b62-prefill-ab-failure-v2"', 1)[1]

...
run_arm() {
  ...
  if [ -n "$extra" ]; then IFS=',' read -ra kv <<< "$extra"
    for x in "${kv[@]}"; do case $x in
      @MAXSEQ=*) for i in "${!argv[@]}"; do [ "${argv[$i]}" = --max-seq-len ] && argv[$((i+1))]=${x#@MAXSEQ=}; done;;
      @PROMPTS=*) prompts=$(echo "${x#@PROMPTS=}" | tr '+' ' ');;
      *) envs+=("$x");;
    esac; done
  fi

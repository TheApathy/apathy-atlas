...
# BEGIN packed E4M3 A/B threshold
if [[ $# -ne 1 ]]; then
    echo "usage: $0 <min_direct_packed_speedup>" >&2
    exit 2
fi
...
if [[ -n ${W2A8_PACKED_E4M3_AB_OUTPUT_DIR:-} ]]; then
    build_dir=$W2A8_PACKED_E4M3_AB_OUTPUT_DIR
    if [[ -e $build_dir || -L $build_dir ]]; then
        echo "refusing existing W2A8_PACKED_E4M3_AB_OUTPUT_DIR: $build_dir" >&2
        exit 2
    fi
    mkdir -m 0700 -- "$build_dir"
    persist_probe=1
else
    build_dir=$(mktemp -d /tmp/atlas-exl3-packed-e4m3-ab.XXXXXX)
...
for tool in "$nvcc_bin" "$cuobjdump_bin" "$host_cxx_bin" "$timeout_bin"; do
    if [[ ! -x $tool ]]; then
        echo "missing packed-E4M3 A/B tool: $tool" >&2
        exit 2
    fi
done

# BEGIN packed E4M3 A/B immutable inputs
...
build_script="$repo_root/scripts/check-exl3-prefill-w2a8-packed-e4m3-ab-probe-build.sh"
dependencies=(
...
for dependency in "${dependencies[@]}"; do
    if [[ ! -f $dependency ]]; then
        echo "missing packed-E4M3 A/B input: $dependency" >&2
        exit 2
    fi
...
# END packed E4M3 A/B immutable inputs

# BEGIN packed E4M3 two binary build
manifest="$build_dir/build-manifest.txt"
{
    echo "receipt_format=atlas-w2a8-packed-e4m3-composed-ab-v1"
    echo "min_direct_packed_speedup=$min_speedup"
    echo "min_direct_packed_speedup_binary64=$min_speedup"
    ...
    echo "variant incumbent compile_define=-DW2A8_PACKED_E4M3_CANDIDATE=0"
    echo "variant candidate compile_define=-DW2A8_PACKED_E4M3_CANDIDATE=1"
    ...
    echo "compile_command_template=<nvcc> -ccbin <host_cxx> -std=c++17 -O3 --fmad=false -arch=sm_121a -Xptxas=-v -DW2A8_PACKED_E4M3_CANDIDATE=<0|1> -DW2A8_COMPOSED_PROBE_ENFORCE_SPEEDUP=0 <receipt-bound macros> <harness> <five production wrappers> -o <binary>"
...
build_variant() {
    ...
    "$nvcc_bin" -ccbin "$host_cxx_bin" -std=c++17 -O3 --fmad=false \
        -arch=sm_121a -Xptxas=-v "-DW2A8_PACKED_E4M3_CANDIDATE=$flag" \
...
for symbol in "${target_symbols[@]}"; do
    incumbent_key="incumbent|$symbol"
    candidate_key="candidate|$symbol"
    (( f16_converts[$incumbent_key] == 0 ))
    (( f32_converts[$incumbent_key] > 0 ))
    (( f16_converts[$candidate_key] > 0 ))
    (( f32_converts[$candidate_key] < f32_converts[$incumbent_key] ))
done

inputs_unchanged=1
for dependency in "${dependencies[@]}"; do
    if [[ $(sha256sum "$dependency" | awk '{print $1}') != "${dependency_hashes[$dependency]}" ]]; then
        echo "packed-E4M3 A/B input changed during compilation: $dependency" >&2
        inputs_unchanged=0
    fi
done
for tool_record in \
    ...
    if [[ $(realpath "$tool") != "$expected_path" ]] ||
        ...
        echo "packed-E4M3 A/B tool changed during compilation: $tool" >&2
        inputs_unchanged=0
    fi
done
...
if [[ $current_commit != "$git_commit" || $current_status_hash != "$git_status_hash" ]]; then
    echo "repository identity changed during packed-E4M3 A/B compilation" >&2
    inputs_unchanged=0
fi
[[ $inputs_unchanged == 1 ]]
...
receipt="$build_dir/build-receipt.txt"
{
    ...
    for variant in incumbent candidate; do
        for symbol in "${target_symbols[@]}"; do
            ...
            echo "sass_census variant=$variant symbol=$symbol instructions=${instructions[$key]} e4m3_f16=${f16_converts[$key]} e4m3_f32=${f32_converts[$key]}"
        done
    done
    for symbol in "${target_symbols[@]}"; do
        incumbent_key="incumbent|$symbol"
        candidate_key="candidate|$symbol"
        echo "ab_delta symbol=$symbol instructions=$((instructions[$candidate_key] - instructions[$incumbent_key])) registers=$((registers[$candidate_key] - registers[$incumbent_key])) shared=$((shared[$candidate_key] - shared[$incumbent_key])) e4m3_f16=$((f16_converts[$candidate_key] - f16_converts[$incumbent_key])) e4m3_f32=$((f32_converts[$candidate_key] - f32_converts[$incumbent_key]))"
    done
    echo "direct_pack_sass=PASS incumbent_f16=0 candidate_f16=present candidate_f32=reduced stack=0 local=0 spills=0"
} >"$receipt"
receipt_hash=$(sha256sum "$receipt" | awk '{print $1}')
# END packed E4M3 two binary build

# BEGIN packed E4M3 generated A/B runner
runner="$build_dir/run-packed-e4m3-ab.sh"
{
...
verify_artifacts() {
    ...
    if [[ $actual_receipt_sha256 != "$expected_receipt_sha256" ]] ||
        ...
        [[ $(grep -Fxc "min_direct_packed_speedup=$expected_min_speedup" "$receipt") -ne 1 ]] ||
        [[ $(sha256sum "$timeout_bin" | awk '{print $1}') != "$expected_timeout_sha256" ]]; then
        echo "packed-E4M3 A/B receipt or tool identity mismatch" >&2
        exit 2
    fi
    while read -r _ relative expected; do
        ...
        if [[ $actual != "$expected" ]]; then
            echo "packed-E4M3 A/B artifact mismatch: $relative" >&2
            exit 2
        fi
...
verify_artifacts
probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/atlas-packed-e4m3-ab-run.XXXXXX")
...
for index in 0 1 2 3; do
    variant=${order[$index]}
    binary="$runner_dir/exl3-w2a8-packed-e4m3-${variant}"
    set +e
    (
        ulimit -f 64
    ...
    if [[ $probe_rc -ne 0 || $stdout_bytes -gt 8192 || $stderr_bytes -ne 0 ||
        ...
        echo "packed-E4M3 $variant run rejected: rc=$probe_rc stdout_bytes=$stdout_bytes stderr_bytes=$stderr_bytes stdout_lines=$stdout_lines" >&2
        exit 1
    fi
    mapfile -t lines <"$probe_tmp/stdout-$index"
    expected_flag=0
    [[ $variant == candidate ]] && expected_flag=1
    if [[ ${lines[0]} != "build_id=$expected_pair_id" ]] ||
        [[ ${lines[1]} != "variant=$variant packed_e4m3=$expected_flag" ]] ||
        ...
        echo "packed-E4M3 $variant output contract mismatch" >&2
        exit 1
    fi
...
for index in 1 2 3; do
    if [[ ${device_lines[$index]} != "${device_lines[0]}" ]] ||
        ...
        echo "packed-E4M3 A/B device, input, or output identity mismatch" >&2
        exit 1
    fi
done

if ! direct_packed_speedup=$(awk \
    ...
    echo "packed-E4M3 ABBA speedup below receipt-bound threshold" >&2
    exit 1
fi
if [[ ! $direct_packed_speedup =~ ^$number$ ]]; then
    preserve_failure "outer-abba-non-finite"
    echo "packed-E4M3 ABBA produced a non-finite speedup" >&2
    exit 1
fi
...
printf 'direct_packed_speedup=%s threshold=%s abba_samples=4\n' \
    "$direct_packed_speedup" "$expected_min_speedup"
...
printf 'raw_bf16=exact final_bf16=exact production_alias=exact\n'
grep -E '^(resource_census|sass_census|ab_delta|direct_pack_sass=)' "$receipt"
printf 'result=PASS\n'
RUNNER
} >"$runner"
chmod 0755 "$runner"
runner_hash=$(sha256sum "$runner" | awk '{print $1}')
# END packed E4M3 generated A/B runner

echo "packed-E4M3 composed W2A8 A/B binaries compile for SM121a"

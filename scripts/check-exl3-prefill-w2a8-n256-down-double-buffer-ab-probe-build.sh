...
# BEGIN N256 down double buffer A/B threshold
if [[ $# -ne 1 ]]; then
    echo "usage: $0 <min_n256_down_double_buffer_speedup>" >&2
    exit 2
fi
...
# END N256 down double buffer A/B threshold
...
if [[ -n ${W2A8_N256_DOWN_DOUBLE_BUFFER_AB_OUTPUT_DIR:-} ]]; then
    build_dir=$W2A8_N256_DOWN_DOUBLE_BUFFER_AB_OUTPUT_DIR
    if [[ -e $build_dir || -L $build_dir ]]; then
        echo "refusing existing W2A8_N256_DOWN_DOUBLE_BUFFER_AB_OUTPUT_DIR: $build_dir" >&2
        exit 2
    fi
    mkdir -m 0700 -- "$build_dir"
    persist_probe=1
else
    build_dir=$(mktemp -d /tmp/atlas-exl3-n256-down-double-buffer-ab.XXXXXX)
...
for tool in "$nvcc_bin" "$cuobjdump_bin" "$host_cxx_bin" "$timeout_bin"; do
    if [[ ! -x $tool ]]; then
        echo "missing N256-down double-buffer A/B tool: $tool" >&2
        exit 2
    fi
done

# BEGIN N256 down double buffer A/B immutable inputs
...
candidate_source_test="$repo_root/crates/spark-model/tests/exl3_w2a8_n256_down_double_buffer_model.rs"
candidate_sass_gate="$repo_root/scripts/check-exl3-prefill-w2a8-n256-down-double-buffer-sass.sh"
build_script="$repo_root/scripts/check-exl3-prefill-w2a8-n256-down-double-buffer-ab-probe-build.sh"
dependencies=(
...
for dependency in "${dependencies[@]}"; do
    if [[ ! -f $dependency ]]; then
        echo "missing N256-down double-buffer A/B input: $dependency" >&2
        exit 2
    fi
...
# END N256 down double buffer A/B immutable inputs
...
# BEGIN N256 down double buffer two binary build
manifest="$build_dir/build-manifest.txt"
{
    echo "receipt_format=atlas-w2a8-n256-down-double-buffer-composed-ab-v1"
    echo "min_n256_down_double_buffer_speedup=$min_speedup"
    echo "min_n256_down_double_buffer_speedup_binary64=$min_speedup"
    ...
    echo "only_ab_compile_factor=W2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE"
    echo "variant incumbent compile_define=-DW2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE=0"
    echo "variant candidate compile_define=-DW2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE=1"
...
[[ ${instructions[$incumbent_n128]} == 2752 ]]
...
[[ ${registers[$incumbent_n256]} == 97 ]]
[[ ${shared[$incumbent_n256]} == 10240 ]]
[[ ${instructions[$incumbent_n256]} == 856 ]]
...
[[ ${ldgsts[$incumbent_n256]} == 0 && ${ldgsts[$candidate_n256]} == 2 ]]
[[ ${ldgdepbar[$incumbent_n256]} == 0 && ${ldgdepbar[$candidate_n256]} == 2 ]]
[[ ${depbar_waits[$incumbent_n256]} == 0 && ${depbar_waits[$candidate_n256]} == 2 ]]
[[ ${zfill[$incumbent_n256]} == 0 && ${zfill[$candidate_n256]} == 1 ]]
...
for dependency in "${dependencies[@]}"; do
    if [[ $(sha256sum "$dependency" | awk '{print $1}') != "${dependency_hashes[$dependency]}" ]]; then
        echo "N256-down double-buffer A/B input changed during compilation: $dependency" >&2
        inputs_unchanged=0
    fi
done
for tool_record in \
    ...
    if [[ $(realpath "$tool") != "$expected_path" ]] ||
        ...
        echo "N256-down double-buffer A/B tool changed during compilation: $tool" >&2
        inputs_unchanged=0
    fi
done
...
if [[ $current_commit != "$git_commit" || $current_status_hash != "$git_status_hash" ]]; then
    echo "repository identity changed during N256-down double-buffer A/B compilation" >&2
    inputs_unchanged=0
fi
[[ $inputs_unchanged == 1 ]]
...
receipt="$build_dir/build-receipt.txt"
{
    ...
    echo "n256_down_double_buffer_sass=PASS packed_e4m3_both=PASS fixed_route_guard0=PASS fixed_fused_n128_double_buffer1=PASS unchanged_n128_sass=PASS changed_n256_sass=PASS async_order=PASS arithmetic_census=exact stack=0 local=0 spills=0 atomics=0"
} >"$receipt"
receipt_hash=$(sha256sum "$receipt" | awk '{print $1}')
# END N256 down double buffer two binary build

# BEGIN N256 down double buffer generated A/B runner
runner="$build_dir/run-n256-down-double-buffer-ab.sh"
{
...
verify_artifacts() {
    ...
    if [[ $actual_receipt_sha256 != "$expected_receipt_sha256" ]] ||
        ...
        [[ $(grep -Fxc "min_n256_down_double_buffer_speedup=$expected_min_speedup" "$receipt") -ne 1 ]] ||
        ...
        [[ $(grep -Fxc "only_ab_compile_factor=W2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE" "$receipt") -ne 1 ]] ||
        [[ $(sha256sum "$timeout_bin" | awk '{print $1}') != "$expected_timeout_sha256" ]]; then
        echo "N256-down double-buffer A/B receipt or tool identity mismatch" >&2
        exit 2
    fi
    while read -r _ relative expected; do
        ...
        if [[ $actual != "$expected" ]]; then
            echo "N256-down double-buffer A/B artifact mismatch: $relative" >&2
            exit 2
        fi
...
verify_artifacts
probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/atlas-n256-down-double-buffer-ab-run.XXXXXX")
...
for index in 0 1 2 3; do
    variant=${order[$index]}
    binary="$runner_dir/exl3-w2a8-n256-down-double-buffer-${variant}"
    set +e
    (
        ulimit -f 64
    ...
    if [[ $probe_rc -ne 0 || $stdout_bytes -gt 8192 || $stderr_bytes -ne 0 ||
        ...
        echo "N256-down double-buffer $variant run rejected: rc=$probe_rc stdout_bytes=$stdout_bytes stderr_bytes=$stderr_bytes stdout_lines=$stdout_lines" >&2
        exit 1
    fi
    ...
    if [[ ${lines[0]} != "build_id=$expected_pair_id" ]] ||
        ...
        echo "N256-down double-buffer $variant output contract mismatch" >&2
        exit 1
    fi
...
for index in 1 2 3; do
    if [[ ${device_lines[$index]} != "${device_lines[0]}" ]] ||
        ...
        echo "N256-down double-buffer A/B device, input, or output identity mismatch" >&2
        exit 1
    fi
done

if ! n256_down_double_buffer_speedup=$(awk \
    ...
    echo "N256-down double-buffer ABBA speedup below receipt-bound threshold" >&2
    exit 1
fi
if [[ ! $n256_down_double_buffer_speedup =~ ^$number$ ]]; then
    preserve_failure "outer-abba-non-finite"
    echo "N256-down double-buffer ABBA produced a non-finite speedup" >&2
    exit 1
fi
...
printf 'n256_down_double_buffer_selectors=incumbent:0,candidate:1 packed_e4m3=1,1 n256_route_guard=0,0 fused_n128_double_buffer=1,1\n'
printf 'incumbent_candidate_ms=%s,%s candidate_candidate_ms=%s,%s\n' \
    "${candidate_ms[0]}" "${candidate_ms[3]}" "${candidate_ms[1]}" "${candidate_ms[2]}"
printf 'n256_down_double_buffer_speedup=%s threshold=%s abba_samples=4\n' \
    "$n256_down_double_buffer_speedup" "$expected_min_speedup"
...
grep -E '^(resource_census|sass_census|ab_delta|cubin_comparison|n256_down_double_buffer_sass=)' "$receipt"
printf 'result=PASS\n'
RUNNER
} >"$runner"
chmod 0755 "$runner"
runner_hash=$(sha256sum "$runner" | awk '{print $1}')
# END N256 down double buffer generated A/B runner

echo "N256-down double-buffer composed W2A8 A/B binaries compile for SM121a"

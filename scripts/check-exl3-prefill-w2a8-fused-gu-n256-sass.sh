...
source_file="$repo_root/kernels/gb10/experiments/exl3_w2a8_fused_gu_down_emit_n256.cu"
probe_dir=$(mktemp -d /tmp/atlas-exl3-w2a8-fused-gu-n256.XXXXXX)
...
if [[ ! -f $source_file ]]; then
    echo "missing fused W2A8 M32xN256 component: $source_file" >&2
    exit 1
fi
...
symbol=exl3_w2a8_fused_gu_down_emit_n256
cubin="$probe_dir/fused-gu-n256.cubin"
log="$probe_dir/fused-gu-n256.log"
resources="$probe_dir/fused-gu-n256.resources"
sass="$probe_dir/fused-gu-n256.sass"
...
expect_compile_failure() {
    ...
    if "$nvcc_bin" -std=c++17 -O3 --fmad=false -arch=sm_121a "$@" \
        ...
        >/dev/null 2>&1; then
        echo "invalid fused W2A8 M32xN256 macro set compiled: $label" >&2
        exit 1
    fi
}
...
expect_compile_failure unsupported_n \
    ...
    -DW2A8_KERNEL_NAME=exl3_w2a8_fused_n256_invalid_n
expect_compile_failure crossed_gu \
    -DW2A8_FIXED_N=2048 -DW2A8_FIXED_K=2048 \
    -DW2A8_KERNEL_NAME=exl3_w2a8_fused_n256_invalid_k
expect_compile_failure crossed_down \
    -DW2A8_FIXED_N=4096 -DW2A8_FIXED_K=4096 \
    -DW2A8_KERNEL_NAME=exl3_w2a8_fused_n256_invalid_shape
...
# SM121 allocation is rounded per warp to 256-register quanta. N256 must not
# consume more allocation than the N128 predecessor's 128-reg/256-thread CTA.
allocated_registers_per_block=$(( ((registers * 32 + 255) / 256) * 256 * (threads / 32) ))
(( allocated_registers_per_block <= register_file_limit ))
(( allocated_registers_per_block <= 32768 ))
...
echo "resource_ceiling: allocated_registers_per_block<=32768 shared<=49152 threads=256 arch=sm_121a"

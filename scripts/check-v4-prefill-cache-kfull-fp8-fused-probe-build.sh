...
if [[ -n ${V4_CACHE_KFULL_PROBE_OUTPUT_DIR:-} ]]; then
    build_dir=$V4_CACHE_KFULL_PROBE_OUTPUT_DIR
    if [[ -e $build_dir || -L $build_dir ]]; then
        echo "refusing existing V4_CACHE_KFULL_PROBE_OUTPUT_DIR: $build_dir" >&2
        exit 2
    fi
    mkdir -m 0700 -- "$build_dir"
    persist_probe=1
else
    build_dir=$(mktemp -d /tmp/atlas-v4-cache-kfull-probe.XXXXXX)
...
# BEGIN K-full cache probe immutable inputs
probe_file="$repo_root/kernels/gb10/experiments/v4_prefill_cache_kfull_fp8_fused_probe.cu"
candidate_file="$repo_root/kernels/gb10/deepseek-v4-flash/nvfp4/v4_prefill_cache_kfull_fp8_fused.cu"
...
host_ops_file="$repo_root/crates/spark-model/src/layers/ops/prefill_attn_a.rs"
host_flow_file="$repo_root/crates/spark-model/src/layers/qwen3_attention/prefill/cache_skip_v4.rs"
build_script="$repo_root/scripts/check-v4-prefill-cache-kfull-fp8-fused-probe-build.sh"
dependencies=("$probe_file" "$candidate_file" "$mla_file" "$reshape_file" \
    "$host_ops_file" "$host_flow_file" "$build_script")
declare -A dependency_hashes
for dependency in "${dependencies[@]}"; do
    if [[ ! -f $dependency ]]; then
        echo "missing V4 K-full cache probe input: $dependency" >&2
        exit 2
    fi
...
# END K-full cache probe immutable inputs

# BEGIN K-full cache probe receipt contract
manifest="$build_dir/build-manifest.txt"
{
    echo "receipt_format=atlas-v4-prefill-cache-kfull-fp8-fused-probe-v1"
    ...
    echo "compile_command_template=<nvcc> -ccbin <host_cxx> -std=c++17 -O3 --fmad=false -arch=sm_121a '-DV4_CACHE_KFULL_PROBE_BUILD_ID=\"<build_id>\"' <probe> -o <binary>"
...
binary="$build_dir/v4-prefill-cache-kfull-fp8-fused-probe"
compile_command="$nvcc_bin -ccbin $host_cxx_bin -std=c++17 -O3 --fmad=false -arch=sm_121a '-DV4_CACHE_KFULL_PROBE_BUILD_ID=\"$build_id\"' $probe_file -o $binary"
...
"$nvcc_bin" -ccbin "$host_cxx_bin" -std=c++17 -O3 --fmad=false -arch=sm_121a \
    "-DV4_CACHE_KFULL_PROBE_BUILD_ID=\"$build_id\"" "$probe_file" -o "$binary"
...
reject_threshold() {
    ...
    set -e
    if [[ $result -ne 2 ]]; then
        echo "K-full cache probe accepted invalid threshold: $label" >&2
        exit 1
    fi
    if [[ $label == usage* ]]; then
...
for symbol in mla_q_rope_extract_batched mla_cache_assemble_batched \
    reshape_and_cache_flash_fp8 v4_prefill_cache_kfull_fp8_fused; do
...
if [[ ${#cubins[@]} -lt 1 ]]; then
    echo "no K-full cache probe cubin extracted" >&2
    exit 1
fi

for dependency in "${dependencies[@]}"; do
    if [[ $(sha256sum "$dependency" | awk '{print $1}') != "${dependency_hashes[$dependency]}" ]]; then
        echo "build input changed during V4 K-full cache probe compilation: $dependency" >&2
        exit 1
    fi
done
for tool_record in \
    ...
    if [[ $(realpath "$tool") != "$expected_path" ]] ||
        ...
        echo "build tool changed during V4 K-full cache probe compilation: $tool" >&2
        exit 1
    fi
done
if [[ $(git -C "$repo_root" rev-parse HEAD) != "$git_commit" ]] ||
    [[ $(git -C "$repo_root" status --porcelain=v1 | sha256sum | awk '{print $1}') != "$git_status_hash" ]]; then
    echo "repository identity changed during V4 K-full cache probe compilation" >&2
    exit 1
fi
...
# END K-full cache probe receipt contract

# BEGIN K-full cache probe runner verification
runner="$build_dir/run-v4-prefill-cache-kfull-fp8-fused-probe.sh"
{
...
binary="$runner_dir/v4-prefill-cache-kfull-fp8-fused-probe"
...
if [[ $actual_receipt_sha256 != "$expected_receipt_sha256" ||
    ...
    echo "V4 K-full cache probe artifact identity mismatch" >&2
    exit 2
fi
while read -r _ relative expected_hash; do
    artifact="$runner_dir/$relative"
    if [[ ! -f $artifact ]]; then
        echo "V4 K-full cache probe cubin missing: $relative" >&2
        exit 2
    fi
    actual_cubin_sha256=$(sha256sum "$artifact" | awk '{print $1}')
    if [[ $actual_cubin_sha256 != "$expected_hash" ]]; then
        echo "V4 K-full cache probe cubin identity mismatch: $relative" >&2
        exit 2
    fi
done < <(grep '^cubin_sha256 ' "$receipt")

probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/atlas-v4-cache-kfull-run.XXXXXX")
...
if [[ $probe_rc -ne 0 || $stdout_bytes -gt $max_output_bytes || $stderr_bytes -ne 0 ||
    ...
    echo "V4 K-full cache probe rejected: rc=$probe_rc stdout_bytes=$stdout_bytes stderr_bytes=$stderr_bytes stdout_lines=$stdout_lines" >&2
    exit 1
fi
...
if [[ ${lines[0]} != "build_id=$expected_build_id" ]] ||
    ...
    [[ ! ${lines[2]} =~ ^input_hash=[0-9a-f]{16}\ slot_hash=[0-9a-f]{16}\ scale_hash=[0-9a-f]{16}$ ]] ||
    [[ ${lines[3]} != 'parity cases=3 poison_cases=2 k_mismatches=0 v_mismatches=0' ]] ||
    [[ ${lines[4]} != 'guards rope_scratch=clean assembly_scratch=clean cache=clean malformed_cases=25 unchanged=25 incremental_bytes=616960 incremental_ctas=603' ]] ||
    ...
    echo "V4 K-full cache probe output contract mismatch" >&2
    exit 1
fi
...
chmod 0555 "$runner"
# END K-full cache probe runner verification

echo "V4 K-full cache promotion probe compiles for SM121a"

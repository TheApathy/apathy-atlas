...
# BEGIN continuous ring A/B threshold
if [[ $# -ne 1 ]]; then
    echo "usage: $0 <min_continuous_ring_speedup>" >&2
    exit 2
fi
...
# END continuous ring A/B threshold
...
if [[ -n ${W2A8_FUSED_GU_N128_CONTINUOUS_RING_AB_OUTPUT_DIR:-} ]]; then
    build_dir=$W2A8_FUSED_GU_N128_CONTINUOUS_RING_AB_OUTPUT_DIR
    if [[ -e $build_dir || -L $build_dir ]]; then
        echo "refusing existing W2A8_FUSED_GU_N128_CONTINUOUS_RING_AB_OUTPUT_DIR: $build_dir" >&2
        exit 2
    fi
    mkdir -m 0700 -- "$build_dir"
    persist_probe=1
else
    build_dir=$(mktemp -d /tmp/atlas-exl3-fused-gu-n128-continuous-ring-ab.XXXXXX)
...
for tool in "$nvcc_bin" "$cuobjdump_bin" "$host_cxx_bin" "$timeout_bin"; do
    if [[ ! -x $tool ]]; then
        echo "missing continuous-ring A/B tool: $tool" >&2
        exit 2
    fi
done

# BEGIN continuous ring A/B immutable inputs
...
ring_model_test="$repo_root/crates/spark-model/tests/exl3_w2a8_fused_gu_n128_continuous_ring_model.rs"
ring_ab_model_test="$repo_root/crates/spark-model/tests/exl3_w2a8_fused_gu_n128_continuous_ring_ab_probe_model.rs"
ring_sass_gate="$repo_root/scripts/check-exl3-prefill-w2a8-fused-gu-n128-continuous-ring-sass.sh"
fused_double_buffer_model_test="$repo_root/crates/spark-model/tests/exl3_w2a8_fused_gu_n128_double_buffer_model.rs"
n256_double_buffer_model_test="$repo_root/crates/spark-model/tests/exl3_w2a8_n256_down_double_buffer_model.rs"
n256_double_buffer_sass_gate="$repo_root/scripts/check-exl3-prefill-w2a8-n256-down-double-buffer-sass.sh"
build_script="$repo_root/scripts/check-exl3-prefill-w2a8-fused-gu-n128-continuous-ring-ab-probe-build.sh"
dependencies=(
    ...
    "$kernel_manifest" "$ring_model_test" "$ring_ab_model_test"
    "$ring_sass_gate" "$fused_double_buffer_model_test"
    "$n256_double_buffer_model_test" "$n256_double_buffer_sass_gate"
    "$build_script"
)
declare -A dependency_hashes
for dependency in "${dependencies[@]}"; do
    if [[ ! -f $dependency ]]; then
        echo "missing continuous-ring A/B input: $dependency" >&2
        exit 2
    fi
...
# END continuous ring A/B immutable inputs

# BEGIN generated experiment-only fused N128 wrapper
fused_experiment_wrapper="$build_dir/fused-n128-continuous-ring-experiment-wrapper.cu"
cat >"$fused_experiment_wrapper" <<GENERATED_FUSED_N128_WRAPPER
// SPDX-License-Identifier: AGPL-3.0-only
...
#error "W2A8_PACKED_E4M3_CANDIDATE must remain fixed at 1"
#endif
#ifndef W2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE
#error "W2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE must be receipt-bound"
#endif
#if W2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE != 1
#error "W2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE must remain fixed at 1"
#endif
#ifndef W2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE
#error "W2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE must be receipt-bound"
#endif
#include "$fused_component"
GENERATED_FUSED_N128_WRAPPER
generated_wrapper_sha256=$(sha256sum "$fused_experiment_wrapper" | awk '{print $1}')
# END generated experiment-only fused N128 wrapper

# BEGIN continuous ring two binary build
manifest="$build_dir/build-manifest.txt"
{
    echo "receipt_format=atlas-w2a8-fused-gu-n128-continuous-ring-composed-ab-v1"
    echo "min_continuous_ring_speedup=$min_speedup"
    echo "min_continuous_ring_speedup_binary64=$min_speedup"
    ...
    echo "only_ab_compile_factor=W2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE"
    echo "variant incumbent compile_define=-DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=0"
    echo "variant candidate compile_define=-DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=1"
    echo "generated_wrapper_sha256=$generated_wrapper_sha256"
    ...
    echo "fused_experiment_wrapper_template=fixed_N2048_K4096_symbol_packed1_double_buffer1_ring_<0|1>_direct_component_include"
    echo "production_fused_wrapper=excluded_from_link"
    echo "compile_command_template=<nvcc> -ccbin <host_cxx> -std=c++17 -O3 --fmad=false -arch=sm_121a -Xptxas=-v -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=0 -DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=<0|1> -DW2A8_COMPOSED_PROBE_ENFORCE_SPEEDUP=0 <receipt-bound macros> <harness> <three production dependency wrappers> <generated fused experiment wrapper> <production N256 wrapper> -o <binary>"
...
artifact_files=("${manifest#"$build_dir/"}" "${fused_experiment_wrapper#"$build_dir/"}")
...
build_variant() {
    ...
    local binary="$build_dir/exl3-w2a8-fused-gu-n128-continuous-ring-${variant}"
    local ptxas="$build_dir/${variant}.ptxas"
    local resources="$build_dir/${variant}.resources"
    local compile_command
    compile_command="$nvcc_bin -ccbin $host_cxx_bin -std=c++17 -O3 --fmad=false -arch=sm_121a -Xptxas=-v -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=0 -DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=$selector -DW2A8_COMPOSED_PROBE_ENFORCE_SPEEDUP=0 '-DW2A8_COMPOSED_PROBE_BUILD_ID=\"$pair_id\"' -DW2A8_COMPOSED_PROBE_MIN_SPEEDUP=1.01 '-DW2A8_COMPOSED_PROBE_MIN_SPEEDUP_TEXT=\"1.01\"' $harness_file $emitter_wrapper $n64_gu_wrapper $n64_down_wrapper $fused_experiment_wrapper $n256_down_wrapper -o $binary"
    ...
    "$nvcc_bin" -ccbin "$host_cxx_bin" -std=c++17 -O3 --fmad=false \
        ...
        "$n64_down_wrapper" "$fused_experiment_wrapper" "$n256_down_wrapper" \
        -o "$binary" 2>"$ptxas"
    ...
    for symbol in "${target_symbols[@]}"; do
        ...
        for value in "${registers[$key]}" "${stack[$key]}" "${local_bytes[$key]}" \
            ...
            "${swiglu_exp[$key]}" "${fma_ops[$key]}" "${fadd[$key]}" \
            "${fmul[$key]}" "${ffma[$key]}" "${hadd2[$key]}" \
            "${hmul2[$key]}" "${ldgsts[$key]}" "${ldgdepbar[$key]}" \
            "${depbar_waits[$key]}" "${zfill[$key]}"; do
            [[ $value =~ ^[0-9]+$ ]]
        done
...
for field in registers shared stack local_bytes instructions f16_converts \
    ...
    fma_ops fadd fmul ffma hadd2 hmul2 ldgsts ldgdepbar depbar_waits zfill; do
    declare -n metric_ref=$field
    [[ ${metric_ref[$incumbent_n256]} == "${metric_ref[$candidate_n256]}" ]]
    unset -n metric_ref
done
[[ ${registers[$incumbent_n128]} == 128 && ${registers[$candidate_n128]} == 128 ]]
[[ ${shared[$incumbent_n128]} == 48128 && ${shared[$candidate_n128]} == 48128 ]]
[[ ${instructions[$incumbent_n128]} == 2752 && ${instructions[$candidate_n128]} == 2784 ]]
[[ ${barriers[$incumbent_n128]} == 7 && ${barriers[$candidate_n128]} == 7 ]]
for field in qmma bf16_rounds fp8_converts f16_converts f32_converts \
    swiglu_exp fma_ops shfl fadd fmul ffma hadd2 hmul2; do
    declare -n metric_ref=$field
...
[[ ${f16_converts[$incumbent_n128]} == 32 ]]
[[ ${f32_converts[$incumbent_n128]} == 4 ]]
[[ ${swiglu_exp[$incumbent_n128]} == 4 ]]
[[ ${fma_ops[$incumbent_n128]} == 166 ]]
[[ ${shfl[$incumbent_n128]} == 193 ]]
[[ ${fadd[$incumbent_n128]} == 116 ]]
[[ ${fmul[$incumbent_n128]} == 128 ]]
[[ ${ffma[$incumbent_n128]} == 166 ]]
[[ ${hadd2[$incumbent_n128]} == 44 ]]
[[ ${hmul2[$incumbent_n128]} == 32 ]]
[[ ${ldgsts[$incumbent_n128]} == 8 && ${ldgsts[$candidate_n128]} == 8 ]]
[[ ${ldgdepbar[$incumbent_n128]} == 4 && ${ldgdepbar[$candidate_n128]} == 4 ]]
[[ ${depbar_waits[$incumbent_n128]} == 4 && ${depbar_waits[$candidate_n128]} == 4 ]]
[[ ${zfill[$incumbent_n128]} == 2 && ${zfill[$candidate_n128]} == 4 ]]
...
awk '
    ...
    /DEPBAR\.LE SB0, 0x0/ {
        ...
        if ((waits == 1 || waits == 3) &&
            overlap_qmma != 16 * ((waits + 1) / 2)) exit 1
        last_wait = NR
        waits++
    }
    /BAR\.SYNC/ {
...
' "$build_dir/candidate.$n128_symbol.sass"

inputs_unchanged=1
if [[ $(sha256sum "$fused_experiment_wrapper" | awk '{print $1}') != \
    "$generated_wrapper_sha256" ]]; then
    echo "generated continuous-ring experiment wrapper changed during compilation" >&2
    inputs_unchanged=0
fi
for dependency in "${dependencies[@]}"; do
    if [[ $(sha256sum "$dependency" | awk '{print $1}') != "${dependency_hashes[$dependency]}" ]]; then
        echo "continuous-ring A/B input changed during compilation: $dependency" >&2
        inputs_unchanged=0
    fi
done
for tool_record in \
    ...
    if [[ $(realpath "$tool") != "$expected_path" ]] ||
        ...
        echo "continuous-ring A/B tool changed during compilation: $tool" >&2
        inputs_unchanged=0
    fi
done
...
if [[ $current_commit != "$git_commit" || $current_status_hash != "$git_status_hash" ]]; then
    echo "repository identity changed during continuous-ring A/B compilation" >&2
    inputs_unchanged=0
fi
[[ $inputs_unchanged == 1 ]]
...
receipt="$build_dir/build-receipt.txt"
{
    ...
    echo "continuous_ring_sass=PASS packed_e4m3_both=PASS fixed_fused_double_buffer_both=PASS fixed_n256_down_double_buffer_both=PASS fixed_n256_route_guard_both=PASS changed_n128_sass=PASS unchanged_n256_sass=PASS async_order=PASS arithmetic_census=exact stack=0 local=0 spills=0 atomics=0"
} >"$receipt"
receipt_hash=$(sha256sum "$receipt" | awk '{print $1}')
# END continuous ring two binary build

# BEGIN continuous ring generated A/B runner
runner="$build_dir/run-fused-gu-n128-continuous-ring-ab.sh"
{
...
verify_artifacts() {
    ...
    if [[ $actual_receipt_sha256 != "$expected_receipt_sha256" ]] ||
        ...
        [[ $(grep -Fxc "min_continuous_ring_speedup=$expected_min_speedup" "$receipt") -ne 1 ]] ||
        ...
        [[ $(grep -Fxc "only_ab_compile_factor=W2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE" "$receipt") -ne 1 ]] ||
        [[ $(grep -Fxc "variant incumbent compile_define=-DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=0" "$receipt") -ne 1 ]] ||
        [[ $(grep -Fxc "variant candidate compile_define=-DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=1" "$receipt") -ne 1 ]] ||
        [[ $(grep -Ec '^compile_command incumbent .* -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=0 -DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=0 .* -o .*/exl3-w2a8-fused-gu-n128-continuous-ring-incumbent$' "$receipt") -ne 1 ]] ||
        [[ $(grep -Ec '^compile_command candidate .* -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_DOWN_DOUBLE_BUFFER_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=0 -DW2A8_FUSED_GU_N128_CONTINUOUS_RING_CANDIDATE=1 .* -o .*/exl3-w2a8-fused-gu-n128-continuous-ring-candidate$' "$receipt") -ne 1 ]] ||
        [[ $(sha256sum "$timeout_bin" | awk '{print $1}') != "$expected_timeout_sha256" ]]; then
        echo "continuous-ring A/B receipt or tool identity mismatch" >&2
        exit 2
    fi
    while read -r _ relative expected; do
        ...
        if [[ $actual != "$expected" ]]; then
            echo "continuous-ring A/B artifact mismatch: $relative" >&2
            exit 2
        fi
...
verify_artifacts
probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/atlas-fused-gu-n128-continuous-ring-ab-run.XXXXXX")
...
for index in 0 1 2 3; do
    variant=${order[$index]}
    binary="$runner_dir/exl3-w2a8-fused-gu-n128-continuous-ring-${variant}"
    set +e
    (
        ulimit -f 64
    ...
    if [[ $probe_rc -ne 0 || $stdout_bytes -gt 8192 || $stderr_bytes -ne 0 ||
        ...
        echo "continuous-ring $variant run rejected: rc=$probe_rc stdout_bytes=$stdout_bytes stderr_bytes=$stderr_bytes stdout_lines=$stdout_lines" >&2
        exit 1
    fi
    ...
    if [[ ${lines[0]} != "build_id=$expected_pair_id" ]] ||
        ...
        echo "continuous-ring $variant output contract mismatch" >&2
        exit 1
    fi
...
for index in 1 2 3; do
    if [[ ${device_lines[$index]} != "${device_lines[0]}" ]] ||
        ...
        echo "continuous-ring A/B device, input, or output identity mismatch" >&2
        exit 1
    fi
done

if ! continuous_ring_speedup=$(awk \
    ...
    echo "continuous-ring ABBA speedup below receipt-bound threshold" >&2
    exit 1
fi
if [[ ! $continuous_ring_speedup =~ ^$number$ ]]; then
    preserve_failure "outer-abba-non-finite"
    echo "continuous-ring ABBA produced a non-finite speedup" >&2
    exit 1
fi
...
printf 'ring_selectors=incumbent:0,candidate:1 packed_e4m3=1,1 fused_double_buffer=1,1 n256_down_double_buffer=1,1 n256_route_guard=0,0\n'
printf '%s\n' "${device_lines[0]}"
printf 'incumbent_candidate_ms=%s,%s candidate_candidate_ms=%s,%s\n' \
    "${candidate_ms[0]}" "${candidate_ms[3]}" "${candidate_ms[1]}" "${candidate_ms[2]}"
printf 'continuous_ring_speedup=%s threshold=%s abba_samples=4\n' \
    "$continuous_ring_speedup" "$expected_min_speedup"
...
# END continuous ring generated A/B runner

echo "continuous-ring composed W2A8 A/B binaries compile for SM121a"

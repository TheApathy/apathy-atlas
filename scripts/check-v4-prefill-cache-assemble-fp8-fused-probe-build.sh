...
if [[ -n ${V4_CACHE_FP8_PROBE_OUTPUT_DIR:-} ]]; then
    build_dir=$V4_CACHE_FP8_PROBE_OUTPUT_DIR
    if [[ -e $build_dir || -L $build_dir ]]; then
        echo "refusing existing V4_CACHE_FP8_PROBE_OUTPUT_DIR: $build_dir" >&2
        exit 2
    fi
    mkdir -m 0700 -- "$build_dir"
    persist_probe=1
else
    build_dir=$(mktemp -d /tmp/atlas-v4-cache-fp8-probe.XXXXXX)
...
# BEGIN cache fusion probe immutable inputs
probe_file="$repo_root/kernels/gb10/experiments/v4_prefill_cache_assemble_fp8_fused_probe.cu"
candidate_file="$repo_root/kernels/gb10/experiments/v4_prefill_cache_assemble_fp8_fused.cu"
...
build_script="$repo_root/scripts/check-v4-prefill-cache-assemble-fp8-fused-probe-build.sh"
dependencies=("$probe_file" "$candidate_file" "$mla_file" "$reshape_file" "$build_script")
declare -A dependency_hashes
for dependency in "${dependencies[@]}"; do
    if [[ ! -f $dependency ]]; then
        echo "missing V4 cache-fusion probe input: $dependency" >&2
        exit 2
    fi
...
# END cache fusion probe immutable inputs

# BEGIN cache fusion probe receipt contract
manifest="$build_dir/build-manifest.txt"
{
    echo "receipt_format=atlas-v4-prefill-cache-assemble-fp8-fused-probe-v1"
    ...
    echo "compile_command_template=<nvcc> -ccbin <host_cxx> -std=c++17 -O3 --fmad=false -arch=sm_121a '-DV4_CACHE_FP8_PROBE_BUILD_ID=\"<build_id>\"' <probe> -o <binary>"
...
binary="$build_dir/v4-prefill-cache-assemble-fp8-fused-probe"
compile_command="$nvcc_bin -ccbin $host_cxx_bin -std=c++17 -O3 --fmad=false -arch=sm_121a '-DV4_CACHE_FP8_PROBE_BUILD_ID=\"$build_id\"' $probe_file -o $binary"
...
"$nvcc_bin" -ccbin "$host_cxx_bin" -std=c++17 -O3 --fmad=false -arch=sm_121a \
    "-DV4_CACHE_FP8_PROBE_BUILD_ID=\"$build_id\"" "$probe_file" -o "$binary"
...
reject_threshold() {
    ...
    set -e
    if [[ $result -ne 2 ]]; then
        echo "cache-fusion probe accepted invalid threshold: $label" >&2
        exit 1
    fi
    if [[ $label == usage* ]]; then
...
for symbol in mla_cache_assemble_batched reshape_and_cache_flash_fp8 \
    v4_prefill_cache_assemble_fp8_fused; do
...
if [[ ${#cubins[@]} -lt 1 ]]; then
    echo "no cache-fusion probe cubin extracted" >&2
    exit 1
fi

for dependency in "${dependencies[@]}"; do
    if [[ $(sha256sum "$dependency" | awk '{print $1}') != "${dependency_hashes[$dependency]}" ]]; then
        echo "build input changed during V4 cache-fusion probe compilation: $dependency" >&2
        exit 1
    fi
done
for tool_record in \
    ...
    if [[ $(realpath "$tool") != "$expected_path" ]] ||
        ...
        echo "build tool changed during V4 cache-fusion probe compilation: $tool" >&2
        exit 1
    fi
done
if [[ $(git -C "$repo_root" rev-parse HEAD) != "$git_commit" ]] ||
    ...
    echo "repository identity changed during V4 cache-fusion probe compilation" >&2
    exit 1
fi
...
# END cache fusion probe receipt contract

# BEGIN cache fusion probe runner verification
runner="$build_dir/run-v4-prefill-cache-assemble-fp8-fused-probe.sh"
{
...
binary="$runner_dir/v4-prefill-cache-assemble-fp8-fused-probe"
...
if [[ $actual_receipt_sha256 != "$expected_receipt_sha256" ||
    ...
    echo "V4 cache-fusion probe artifact identity mismatch" >&2
    exit 2
fi
while read -r _ relative expected_hash; do
    ...
    if [[ ! -f $artifact || $actual_cubin_sha256 != "$expected_hash" ]]; then
        echo "V4 cache-fusion probe cubin identity mismatch: $relative" >&2
        exit 2
    fi
done < <(grep '^cubin_sha256 ' "$receipt")

probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/atlas-v4-cache-fp8-run.XXXXXX")
...
if [[ $probe_rc -ne 0 || $stdout_bytes -gt $max_output_bytes || $stderr_bytes -ne 0 ||
    ...
    echo "V4 cache-fusion probe rejected: rc=$probe_rc stdout_bytes=$stdout_bytes stderr_bytes=$stderr_bytes stdout_lines=$stdout_lines" >&2
    exit 1
fi
...
if [[ ${lines[0]} != "build_id=$expected_build_id" ]] ||
    ...
    [[ ${lines[4]} != 'guards scratch=clean cache=clean malformed_cases=25 unchanged=25' ]] ||
    ...
    echo "V4 cache-fusion probe output contract mismatch" >&2
    exit 1
fi
...
chmod 0555 "$runner"
# END cache fusion probe runner verification

echo "V4 cache-fusion promotion probe compiles for SM121a"

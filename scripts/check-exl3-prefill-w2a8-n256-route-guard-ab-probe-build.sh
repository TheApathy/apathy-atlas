...
# BEGIN N256 route guard A/B threshold
if [[ $# -ne 1 ]]; then
    echo "usage: $0 <min_route_guard_speedup>" >&2
    exit 2
fi
...
# END N256 route guard A/B threshold
...
if [[ -n ${W2A8_N256_ROUTE_GUARD_AB_OUTPUT_DIR:-} ]]; then
    build_dir=$W2A8_N256_ROUTE_GUARD_AB_OUTPUT_DIR
    if [[ -e $build_dir || -L $build_dir ]]; then
        echo "refusing existing W2A8_N256_ROUTE_GUARD_AB_OUTPUT_DIR: $build_dir" >&2
        exit 2
    fi
    mkdir -m 0700 -- "$build_dir"
    persist_probe=1
else
    build_dir=$(mktemp -d /tmp/atlas-exl3-n256-route-guard-ab.XXXXXX)
...
for tool in "$nvcc_bin" "$cuobjdump_bin" "$host_cxx_bin" "$timeout_bin"; do
    if [[ ! -x $tool ]]; then
        echo "missing N256 route-guard A/B tool: $tool" >&2
        exit 2
    fi
done

# BEGIN N256 route guard A/B immutable inputs
...
build_script="$repo_root/scripts/check-exl3-prefill-w2a8-n256-route-guard-ab-probe-build.sh"
dependencies=(
...
for dependency in "${dependencies[@]}"; do
    if [[ ! -f $dependency ]]; then
        echo "missing N256 route-guard A/B input: $dependency" >&2
        exit 2
    fi
...
# END N256 route guard A/B immutable inputs

# BEGIN N256 route guard two binary build
manifest="$build_dir/build-manifest.txt"
{
    echo "receipt_format=atlas-w2a8-n256-route-guard-composed-ab-v1"
    echo "min_route_guard_speedup=$min_speedup"
    echo "min_route_guard_speedup_binary64=$min_speedup"
    ...
    echo "only_ab_compile_factor=W2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE"
    echo "variant incumbent compile_define=-DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=0"
    echo "variant candidate compile_define=-DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=1"
    ...
    echo "compile_command_template=<nvcc> -ccbin <host_cxx> -std=c++17 -O3 --fmad=false -arch=sm_121a -Xptxas=-v -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=<0|1> -DW2A8_COMPOSED_PROBE_ENFORCE_SPEEDUP=0 <receipt-bound macros> <harness> <five production wrappers> -o <binary>"
...
build_variant() {
    ...
    local binary="$build_dir/exl3-w2a8-n256-route-guard-${variant}"
    local ptxas="$build_dir/${variant}.ptxas"
    local resources="$build_dir/${variant}.resources"
    local compile_command
    compile_command="$nvcc_bin -ccbin $host_cxx_bin -std=c++17 -O3 --fmad=false -arch=sm_121a -Xptxas=-v -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=$selector -DW2A8_COMPOSED_PROBE_ENFORCE_SPEEDUP=0 '-DW2A8_COMPOSED_PROBE_BUILD_ID=\"$pair_id\"' -DW2A8_COMPOSED_PROBE_MIN_SPEEDUP=1.01 '-DW2A8_COMPOSED_PROBE_MIN_SPEEDUP_TEXT=\"1.01\"' $harness_file $emitter_wrapper $n64_gu_wrapper $n64_down_wrapper $fused_wrapper $n256_down_wrapper -o $binary"
    ...
    "$nvcc_bin" -ccbin "$host_cxx_bin" -std=c++17 -O3 --fmad=false \
        ...
        "$n64_down_wrapper" "$fused_wrapper" "$n256_down_wrapper" \
        -o "$binary" 2>"$ptxas"
    ...
    for symbol in "${target_symbols[@]}"; do
        ...
        shfl[$key]=$(grep -cE 'SHFL\.IDX' "$sass" || true)
...
[[ ${f32_converts[$incumbent_n128]} == "${f32_converts[$candidate_n128]}" ]]
[[ ${f32_converts[$incumbent_n256]} == 0 && ${f32_converts[$candidate_n256]} == 0 ]]
[[ ${registers[$incumbent_n256]} == 97 ]]
[[ ${registers[$candidate_n256]} == 97 ]]
[[ ${shared[$incumbent_n256]} == 10240 ]]
[[ ${shared[$candidate_n256]} == 10256 ]]
[[ ${instructions[$incumbent_n256]} == 856 ]]
[[ ${instructions[$candidate_n256]} == 872 ]]
[[ ${qmma[$incumbent_n256]} == 16 && ${qmma[$candidate_n256]} == 16 ]]
[[ ${shfl[$incumbent_n256]} == 16 && ${shfl[$candidate_n256]} == 16 ]]
(( barriers[$candidate_n256] == barriers[$incumbent_n256] + 1 ))

inputs_unchanged=1
for dependency in "${dependencies[@]}"; do
    if [[ $(sha256sum "$dependency" | awk '{print $1}') != "${dependency_hashes[$dependency]}" ]]; then
        echo "N256 route-guard A/B input changed during compilation: $dependency" >&2
        inputs_unchanged=0
    fi
done
for tool_record in \
    ...
    if [[ $(realpath "$tool") != "$expected_path" ]] ||
        ...
        echo "N256 route-guard A/B tool changed during compilation: $tool" >&2
        inputs_unchanged=0
    fi
done
...
if [[ $current_commit != "$git_commit" || $current_status_hash != "$git_status_hash" ]]; then
    echo "repository identity changed during N256 route-guard A/B compilation" >&2
    inputs_unchanged=0
fi
[[ $inputs_unchanged == 1 ]]
...
receipt="$build_dir/build-receipt.txt"
{
    ...
    for variant in incumbent candidate; do
        for symbol in "${target_symbols[@]}"; do
            ...
            echo "sass_census variant=$variant symbol=$symbol instructions=${instructions[$key]} e4m3_f16=${f16_converts[$key]} e4m3_f32=${f32_converts[$key]} barriers=${barriers[$key]} qmma=${qmma[$key]} shfl=${shfl[$key]}"
        done
    done
    ...
    echo "route_guard_sass=PASS packed_e4m3_both=PASS unchanged_n128_sass=PASS changed_n256_sass=PASS stack=0 local=0 spills=0"
} >"$receipt"
receipt_hash=$(sha256sum "$receipt" | awk '{print $1}')
# END N256 route guard two binary build

# BEGIN N256 route guard generated A/B runner
runner="$build_dir/run-n256-route-guard-ab.sh"
{
...
verify_artifacts() {
    ...
    if [[ $actual_receipt_sha256 != "$expected_receipt_sha256" ]] ||
        ...
        [[ $(grep -Fxc "min_route_guard_speedup=$expected_min_speedup" "$receipt") -ne 1 ]] ||
        [[ $(grep -Fxc "packed_e4m3=enabled_both_arms" "$receipt") -ne 1 ]] ||
        [[ $(grep -Fxc "only_ab_compile_factor=W2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE" "$receipt") -ne 1 ]] ||
        [[ $(sha256sum "$timeout_bin" | awk '{print $1}') != "$expected_timeout_sha256" ]]; then
        echo "N256 route-guard A/B receipt or tool identity mismatch" >&2
        exit 2
    fi
    while read -r _ relative expected; do
        ...
        if [[ $actual != "$expected" ]]; then
            echo "N256 route-guard A/B artifact mismatch: $relative" >&2
            exit 2
        fi
...
verify_artifacts
probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/atlas-n256-route-guard-ab-run.XXXXXX")
...
for index in 0 1 2 3; do
    variant=${order[$index]}
    binary="$runner_dir/exl3-w2a8-n256-route-guard-${variant}"
    set +e
    (
        ulimit -f 64
    ...
    if [[ $probe_rc -ne 0 || $stdout_bytes -gt 8192 || $stderr_bytes -ne 0 ||
        ...
        echo "N256 route-guard $variant run rejected: rc=$probe_rc stdout_bytes=$stdout_bytes stderr_bytes=$stderr_bytes stdout_lines=$stdout_lines" >&2
        exit 1
    fi
    ...
    if [[ ${lines[0]} != "build_id=$expected_pair_id" ]] ||
        ...
        echo "N256 route-guard $variant output contract mismatch" >&2
        exit 1
    fi
...
for index in 1 2 3; do
    if [[ ${device_lines[$index]} != "${device_lines[0]}" ]] ||
        ...
        echo "N256 route-guard A/B device, input, or output identity mismatch" >&2
        exit 1
    fi
done

if ! route_guard_speedup=$(awk \
    ...
    echo "N256 route-guard ABBA speedup below receipt-bound threshold" >&2
    exit 1
fi
if [[ ! $route_guard_speedup =~ ^$number$ ]]; then
    preserve_failure "outer-abba-non-finite"
    echo "N256 route-guard ABBA produced a non-finite speedup" >&2
    exit 1
fi
...
printf 'selectors=incumbent:0,candidate:1 packed_e4m3=1,1\n'
printf 'incumbent_candidate_ms=%s,%s candidate_candidate_ms=%s,%s\n' \
    "${candidate_ms[0]}" "${candidate_ms[3]}" "${candidate_ms[1]}" "${candidate_ms[2]}"
printf 'route_guard_speedup=%s threshold=%s abba_samples=4\n' \
    "$route_guard_speedup" "$expected_min_speedup"
...
grep -E '^(resource_census|sass_census|ab_delta|cubin_comparison|route_guard_sass=)' "$receipt"
printf 'result=PASS\n'
RUNNER
} >"$runner"
chmod 0755 "$runner"
runner_hash=$(sha256sum "$runner" | awk '{print $1}')
# END N256 route guard generated A/B runner

echo "N256 route-guard composed W2A8 A/B binaries compile for SM121a"
echo "packed E4M3 is enabled in both arms; only the route-guard selector differs"

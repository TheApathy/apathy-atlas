...
source_file="$repo_root/kernels/gb10/deepseek-v4-flash/nvfp4/v4_prefill_cache_kfull_fp8_fused.cu"
probe_dir=$(mktemp -d /tmp/atlas-v4-cache-kfull-fp8-fused.XXXXXX)
...
if [[ ! -f $source_file ]]; then
    echo "missing isolated V4 strided-K cache source: $source_file" >&2
    exit 1
fi

cubin="$probe_dir/v4-prefill-cache-kfull-fp8-fused.cubin"
log="$probe_dir/v4-prefill-cache-kfull-fp8-fused.log"
resources="$probe_dir/v4-prefill-cache-kfull-fp8-fused.resources"
sass="$probe_dir/v4-prefill-cache-kfull-fp8-fused.sass"
...
symbol=v4_prefill_cache_kfull_fp8_fused

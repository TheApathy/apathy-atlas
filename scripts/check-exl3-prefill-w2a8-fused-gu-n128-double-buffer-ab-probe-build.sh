...
# BEGIN fused N128 double buffer A/B threshold
if [[ $# -ne 1 ]]; then
    echo "usage: $0 <min_double_buffer_speedup>" >&2
    exit 2
fi
...
# END fused N128 double buffer A/B threshold
...
if [[ -n ${W2A8_FUSED_GU_N128_DOUBLE_BUFFER_AB_OUTPUT_DIR:-} ]]; then
    build_dir=$W2A8_FUSED_GU_N128_DOUBLE_BUFFER_AB_OUTPUT_DIR
    if [[ -e $build_dir || -L $build_dir ]]; then
        echo "refusing existing W2A8_FUSED_GU_N128_DOUBLE_BUFFER_AB_OUTPUT_DIR: $build_dir" >&2
        exit 2
    fi
    mkdir -m 0700 -- "$build_dir"
    persist_probe=1
else
    build_dir=$(mktemp -d /tmp/atlas-exl3-fused-gu-n128-double-buffer-ab.XXXXXX)
...
for tool in "$nvcc_bin" "$cuobjdump_bin" "$host_cxx_bin" "$timeout_bin"; do
    if [[ ! -x $tool ]]; then
        echo "missing fused-N128 double-buffer A/B tool: $tool" >&2
        exit 2
    fi
done

# BEGIN fused N128 double buffer A/B immutable inputs
...
build_script="$repo_root/scripts/check-exl3-prefill-w2a8-fused-gu-n128-double-buffer-ab-probe-build.sh"
dependencies=(
    ...
    "$kernel_manifest" "$build_script"
)
declare -A dependency_hashes
for dependency in "${dependencies[@]}"; do
    if [[ ! -f $dependency ]]; then
        echo "missing fused-N128 double-buffer A/B input: $dependency" >&2
        exit 2
    fi
...
# END fused N128 double buffer A/B immutable inputs

# BEGIN fused N128 double buffer two binary build
manifest="$build_dir/build-manifest.txt"
{
    echo "receipt_format=atlas-w2a8-fused-gu-n128-double-buffer-composed-ab-v1"
    echo "min_double_buffer_speedup=$min_speedup"
    echo "min_double_buffer_speedup_binary64=$min_speedup"
    ...
    echo "only_ab_compile_factor=W2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE"
    echo "variant incumbent compile_define=-DW2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE=0"
    echo "variant candidate compile_define=-DW2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE=1"
    ...
    echo "fused_ab_wrapper_template=fixed_N2048_K4096_symbol_packed1_double_buffer_<0|1>_direct_component_include"
    echo "compile_command_template=<nvcc> -ccbin <host_cxx> -std=c++17 -O3 --fmad=false -arch=sm_121a -Xptxas=-v -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=0 -DW2A8_COMPOSED_PROBE_ENFORCE_SPEEDUP=0 <receipt-bound macros> <harness> <four production wrappers> <generated fused A/B wrapper> -o <binary>"
...
declare -A barriers qmma shfl sass_hashes cubin_hashes cubin_paths cubin_counts
declare -A bf16_rounds fp8_converts swiglu_exp fma_ops
declare -A ldgsts ldgdepbar depbar_waits zfill
artifact_files=("${manifest#"$build_dir/"}")
...
build_variant() {
    ...
    local binary="$build_dir/exl3-w2a8-fused-gu-n128-double-buffer-${variant}"
    local ptxas="$build_dir/${variant}.ptxas"
    local resources="$build_dir/${variant}.resources"
    local fused_ab_wrapper="$build_dir/${variant}.fused-ab-wrapper.cu"
    local compile_command
    {
        ...
        printf '#define W2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE %s\n' "$selector"
        printf '#include "%s"\n' "$fused_component"
    } >"$fused_ab_wrapper"
    artifact_files+=("${fused_ab_wrapper#"$build_dir/"}")
    compile_command="$nvcc_bin -ccbin $host_cxx_bin -std=c++17 -O3 --fmad=false -arch=sm_121a -Xptxas=-v -DW2A8_PACKED_E4M3_CANDIDATE=1 -DW2A8_N256_SINGLE_WARP_ROUTE_GUARD_CANDIDATE=0 -DW2A8_COMPOSED_PROBE_ENFORCE_SPEEDUP=0 '-DW2A8_COMPOSED_PROBE_BUILD_ID=\"$pair_id\"' -DW2A8_COMPOSED_PROBE_MIN_SPEEDUP=1.01 '-DW2A8_COMPOSED_PROBE_MIN_SPEEDUP_TEXT=\"1.01\"' $harness_file $emitter_wrapper $n64_gu_wrapper $n64_down_wrapper $fused_ab_wrapper $n256_down_wrapper -o $binary"
    ...
    "$nvcc_bin" -ccbin "$host_cxx_bin" -std=c++17 -O3 --fmad=false \
        ...
        "$n64_down_wrapper" "$fused_ab_wrapper" "$n256_down_wrapper" \
        -o "$binary" 2>"$ptxas"
    ...
    for symbol in "${target_symbols[@]}"; do
        ...
        for value in "${registers[$key]}" "${stack[$key]}" "${local_bytes[$key]}" \
            ...
            "${ldgdepbar[$key]}" "${depbar_waits[$key]}" "${zfill[$key]}"; do
            [[ $value =~ ^[0-9]+$ ]]
        done
...
for field in registers shared stack local_bytes instructions f16_converts \
    ...
    fma_ops ldgsts ldgdepbar depbar_waits zfill; do
...
[[ ${shared[$incumbent_n128]} == 40960 && ${shared[$candidate_n128]} == 48128 ]]
(( instructions[$candidate_n128] > instructions[$incumbent_n128] ))
(( barriers[$candidate_n128] == barriers[$incumbent_n128] ))
for field in qmma bf16_rounds fp8_converts swiglu_exp fma_ops shfl; do
    declare -n metric_ref=$field
...
[[ ${ldgsts[$incumbent_n128]} == 0 && ${ldgsts[$candidate_n128]} == 8 ]]
[[ ${ldgdepbar[$incumbent_n128]} == 0 && ${ldgdepbar[$candidate_n128]} == 4 ]]
[[ ${depbar_waits[$incumbent_n128]} == 0 && ${depbar_waits[$candidate_n128]} == 4 ]]
[[ ${zfill[$incumbent_n128]} == 0 && ${zfill[$candidate_n128]} == 2 ]]
...
for dependency in "${dependencies[@]}"; do
    if [[ $(sha256sum "$dependency" | awk '{print $1}') != "${dependency_hashes[$dependency]}" ]]; then
        echo "fused-N128 double-buffer A/B input changed during compilation: $dependency" >&2
        inputs_unchanged=0
    fi
done
for tool_record in \
    ...
    if [[ $(realpath "$tool") != "$expected_path" ]] ||
        ...
        echo "fused-N128 double-buffer A/B tool changed during compilation: $tool" >&2
        inputs_unchanged=0
    fi
done
...
if [[ $current_commit != "$git_commit" || $current_status_hash != "$git_status_hash" ]]; then
    echo "repository identity changed during fused-N128 double-buffer A/B compilation" >&2
    inputs_unchanged=0
fi
[[ $inputs_unchanged == 1 ]]
...
receipt="$build_dir/build-receipt.txt"
{
    ...
    for variant in incumbent candidate; do
        for symbol in "${target_symbols[@]}"; do
            ...
            echo "sass_census variant=$variant symbol=$symbol instructions=${instructions[$key]} e4m3_f16=${f16_converts[$key]} e4m3_f32=${f32_converts[$key]} barriers=${barriers[$key]} qmma=${qmma[$key]} bf16_rounds=${bf16_rounds[$key]} fp8_converts=${fp8_converts[$key]} swiglu_exp=${swiglu_exp[$key]} fma=${fma_ops[$key]} shfl=${shfl[$key]} ldgsts=${ldgsts[$key]} ldgdepbar=${ldgdepbar[$key]} depbar_waits=${depbar_waits[$key]} zfill=${zfill[$key]}"
        done
    done
    ...
    echo "double_buffer_sass=PASS packed_e4m3_both=PASS changed_n128_sass=PASS unchanged_n256_sass=PASS async_order=PASS arithmetic_census=exact stack=0 local=0 spills=0 atomics=0"
} >"$receipt"
receipt_hash=$(sha256sum "$receipt" | awk '{print $1}')
# END fused N128 double buffer two binary build

# BEGIN fused N128 double buffer generated A/B runner
runner="$build_dir/run-fused-gu-n128-double-buffer-ab.sh"
{
...
verify_artifacts() {
    ...
    if [[ $actual_receipt_sha256 != "$expected_receipt_sha256" ]] ||
        ...
        [[ $(grep -Fxc "min_double_buffer_speedup=$expected_min_speedup" "$receipt") -ne 1 ]] ||
        [[ $(grep -Fxc "packed_e4m3=enabled_both_arms" "$receipt") -ne 1 ]] ||
        [[ $(grep -Fxc "fixed_n256_route_guard=0" "$receipt") -ne 1 ]] ||
        [[ $(grep -Fxc "only_ab_compile_factor=W2A8_FUSED_GU_N128_DOUBLE_BUFFER_CANDIDATE" "$receipt") -ne 1 ]] ||
        [[ $(sha256sum "$timeout_bin" | awk '{print $1}') != "$expected_timeout_sha256" ]]; then
        echo "fused-N128 double-buffer A/B receipt or tool identity mismatch" >&2
        exit 2
    fi
    while read -r _ relative expected; do
        ...
        if [[ $actual != "$expected" ]]; then
            echo "fused-N128 double-buffer A/B artifact mismatch: $relative" >&2
            exit 2
        fi
...
verify_artifacts
probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/atlas-fused-gu-n128-double-buffer-ab-run.XXXXXX")
...
for index in 0 1 2 3; do
    variant=${order[$index]}
    binary="$runner_dir/exl3-w2a8-fused-gu-n128-double-buffer-${variant}"
    set +e
    (
        ulimit -f 64
    ...
    if [[ $probe_rc -ne 0 || $stdout_bytes -gt 8192 || $stderr_bytes -ne 0 ||
        ...
        echo "fused-N128 double-buffer $variant run rejected: rc=$probe_rc stdout_bytes=$stdout_bytes stderr_bytes=$stderr_bytes stdout_lines=$stdout_lines" >&2
        exit 1
    fi
    ...
    if [[ ${lines[0]} != "build_id=$expected_pair_id" ]] ||
        ...
        echo "fused-N128 double-buffer $variant output contract mismatch" >&2
        exit 1
    fi
...
for index in 1 2 3; do
    if [[ ${device_lines[$index]} != "${device_lines[0]}" ]] ||
        ...
        echo "fused-N128 double-buffer A/B device, input, or output identity mismatch" >&2
        exit 1
    fi
done

if ! double_buffer_speedup=$(awk \
    ...
    echo "fused-N128 double-buffer ABBA speedup below receipt-bound threshold" >&2
    exit 1
fi
if [[ ! $double_buffer_speedup =~ ^$number$ ]]; then
    preserve_failure "outer-abba-non-finite"
    echo "fused-N128 double-buffer ABBA produced a non-finite speedup" >&2
    exit 1
fi
...
printf 'double_buffer_selectors=incumbent:0,candidate:1 packed_e4m3=1,1 n256_route_guard=0,0\n'
printf 'incumbent_candidate_ms=%s,%s candidate_candidate_ms=%s,%s\n' \
    "${candidate_ms[0]}" "${candidate_ms[3]}" "${candidate_ms[1]}" "${candidate_ms[2]}"
printf 'double_buffer_speedup=%s threshold=%s abba_samples=4\n' \
    "$double_buffer_speedup" "$expected_min_speedup"
...
grep -E '^(resource_census|sass_census|ab_delta|cubin_comparison|double_buffer_sass=)' "$receipt"
printf 'result=PASS\n'
RUNNER
} >"$runner"
chmod 0755 "$runner"
runner_hash=$(sha256sum "$runner" | awk '{print $1}')
# END fused N128 double buffer generated A/B runner

echo "fused-N128 double-buffer composed W2A8 A/B binaries compile for SM121a"
echo "packed E4M3 is enabled in both arms; only the double-buffer selector differs"

...
start_server() {
  local log=$1; shift
  setsid bash -c "cd $W && exec env -i HOME=$HOME PATH=/usr/local/cuda-13.0/bin:/usr/bin:/bin LD_LIBRARY_PATH=/usr/local/cuda-13.0/lib64 \
      \$(grep -v '^#' $ENVF | xargs -d '\n') $* timeout -s INT -k 60 $SERVER_TIMEOUT $PREFIX $BIN \$(cat $W/bench-verify/argv.p35 | tr '\n' ' ')" > $log 2>&1 &
  SERVER_PGID=$!

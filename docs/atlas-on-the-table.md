...
| 3 | **Acceptance bimodality** — 20–33% of steps accept NOTHING | `ATLAS_DSPARK_ACCEPT_LOG`: 2.52–2.83 tok/step | 2.68 → ~3.08 tok/step = 1.17× → 1.34× multiplier | open — A/B'd 2026-08-09: bit-exact o-proj alone does NOT recover it (2.54 vs 2.83 control, confounded by divergent text; `_ld` also FASTER end-to-end, repeat 31.2 vs 28.4 — so verify default reverted to `_ld`, exact path is opt-in `ATLAS_OPROJ_BATCH_EXACT=1`). Next: bit-exact m=6 MoE leg to complete the exact capture chain, THEN re-A/B; and time grouped_batchm vs `_ld` head-to-head in the microtest (never measured) |

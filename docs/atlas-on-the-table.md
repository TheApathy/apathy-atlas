...
| 3 | **Acceptance bimodality** — 31–33% of steps accept NOTHING | `ATLAS_DSPARK_ACCEPT_LOG`: 2.52–2.68 tok/step | 2.68 → ~3.08 tok/step = 1.17× → 1.34× multiplier | open — NARROWED 2026-08-09: bit-exact o-proj alone does NOT move it (2.54 measured); residual capture drift is in the OTHER reordered verify kernels (m=6 dedup MoE k-order, batched attention). Next: bit-exact m6 MoE, or capture from a plain-order recompute |

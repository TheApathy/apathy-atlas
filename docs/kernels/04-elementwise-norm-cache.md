...
| 2 | `y_out` / `hidden` `[1,4096]` BF16, 8 KiB | `hc_pre_finish` `hyper_connection.cu:291` | `rms_norm_vanilla` `rms_norm_vanilla.cu:37` via the DeepSeek-V4 norm handle | **~1.3 MiB ⇒ 5 µs** + 86 launches | `hc_pre_finish` writes the collapsed hidden vector and `rms_norm_vanilla` reads it back on the very next launch, applying DeepSeek-V4's vanilla `x * rms * weight` semantics without an offset. The strict default-off production arm `ATLAS_V4_PREFILL_HC_RMS_FUSED=1` retains the BF16 `hidden` store while folding the immediate RMS reread/reduction into the finish CTA. At N=2,410 it models 1.581 GiB of avoided logical reads and 86 fewer launches per pass; its 2026-08-27 byte-exact GB10 gate passed at 1.213781x, while end-to-end qualification remains pending. **Highest-confidence structural fusion in this document.** |
